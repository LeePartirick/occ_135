...
var bufferview = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		for(var i=0; i<rows.length; i++) {
			...
			var arr_tmp=rowId.split('-');
			arr_tmp[3]=2;
			var row_height=$('#'+arr_tmp.join('-')).height();
			
			var style = styleValue ? 'style="height:'+row_height+'px;' + styleValue + '"' : 'style="height:'+row_height+'px;"';
...
//		var rows = state.data.rows.slice(this.renderedCount, this.renderedCount+opts.pageSize);
	...
	populate: function(target){
		...
		if (this.rows.length){
			...
			JSON.parse(JSON.stringify(data_other), function (key, value) {   
				if(key != 'rows')
				json[key]=value;
			})
			...
			opts.onLoadSuccess.call(target, json);
