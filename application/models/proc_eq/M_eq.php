 ...
 *  
    设备
 */
class M_eq extends CI_Model {
	
	//@todo 主表配置
	private $table_name='sm_equipment';
	private $pk_id='eq_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='设备';
	private $model_name = 'm_eq';
	private $url_conf = 'proc_eq/eq/edit';
	private $proc_id = 'proc_eq';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_EQ') ) return;
    	define('LOAD_M_EQ', 1);
    	
    	//define

		//设备类别
		define('EQ_SORT_ROUT', 1);
		define('EQ_SORT_SERVER', 2);
		define('EQ_SORT_STORAGE', 3);
		define('EQ_SORT_SECURITY', 4);
		define('EQ_SORT_INTERNET', 5);
		define('EQ_SORT_SOFTWARE', 6);
		define('EQ_SORT_TERMINAL', 7);
		define('EQ_SORT_WEAK', 8);
		define('EQ_SORT_YCWB', 9);
		define('EQ_SORT_THIRDWB', 10);
		define('EQ_SORT_YFWB', 11);
		define('EQ_SORT_YFRLWB', 12);
		define('EQ_SORT_FUWUWB', 13);
		define('EQ_SORT_FUWURLWB', 14);
		define('EQ_SORT_GCFB', 15);
		define('EQ_SORT_GKSB', 16);
		define('EQ_SORT_OTHER', 17);

		$GLOBALS['m_eq']['text']['eq_sort']=array(
			EQ_SORT_ROUT=> '路由',
			EQ_SORT_SERVER=> '服务器',
			EQ_SORT_STORAGE=>'存储设备',
			EQ_SORT_SECURITY=>'安全',
			EQ_SORT_INTERNET=>'网络',
			EQ_SORT_SOFTWARE=>'软件',
			EQ_SORT_TERMINAL=>'终端',
			EQ_SORT_WEAK=>'弱电',
			EQ_SORT_YCWB=>'原厂维保',
			EQ_SORT_THIRDWB=>'第三方维保',
			EQ_SORT_YFWB=>'研发外包',
			EQ_SORT_YFRLWB=>'研发人力外包',
			EQ_SORT_FUWUWB=>'服务外包',
			EQ_SORT_FUWURLWB=>'服务人力外包',
			EQ_SORT_GCFB=>'工程外包',
			EQ_SORT_GKSB=>'工控设备',
			EQ_SORT_OTHER=>'其他',
		);
		...
		//设备类型
		define('EQ_TYPE_PRODUCT', 1);
		define('EQ_TYPE_ZHUANYOU', 2);
		define('EQ_TYPE_TONGYONG', 3);
		define('EQ_TYPE_GONGKONG', 4);
		define('EQ_TYPE_CLOUD', 5);

		$GLOBALS['m_eq']['text']['eq_type']=array(
			EQ_TYPE_PRODUCT=>'产品',
			EQ_TYPE_ZHUANYOU=>'专有配件',
			EQ_TYPE_TONGYONG=>'通用配件',
			EQ_TYPE_GONGKONG=>'工控产品',
			EQ_TYPE_CLOUD=>'云平台',
		);

		//合作关系
		define('EQ_PARTNERSHIP_A', 1);
		define('EQ_PARTNERSHIP_B', 2);
		define('EQ_PARTNERSHIP_C', 3);


		$GLOBALS['m_eq']['text']['eq_partnership']=array(
			EQ_PARTNERSHIP_A=>'A',
			EQ_PARTNERSHIP_B=>'B',
			EQ_PARTNERSHIP_C=>'C'
		);
	...
	public function check_acl( $data_db=array() ,$acl_list = NULL)
	{
		...
		if( ! $acl_list )
			$acl_list= $this->m_proc_eq->get_acl();

		$msg='';
		/************权限验证*****************/

		//如果有超级权限，TRUE
		if( ($acl_list & pow(2,ACL_PROC_EQ_SUPER)) != 0 )
		{
			return TRUE;
		}
		...
		if( ! $check_acl
			&& ($acl_list & pow(2,ACL_PROC_EQ_USER)) != 0
		)
		{
			$check_acl=TRUE;
		}
	...
	public function create_import_xlsx()
    {
    	...
    	$conf['field_edit']=array(
			'sm_equipment[eq_brand]',
			'sm_equipment[eq_name]',
			'sm_equipment[eq_model]',
			'sm_equipment[eq_sort]',
			'sm_equipment[eq_unit]',
			'sm_equipment[eq_maintenance]',
			'sm_equipment[eq_description]',
			'sm_equipment[eq_parameter]',
			'sm_equipment[eq_type]',
			'sm_equipment[eq_price]',
			'sm_equipment[eq_org]',
			'sm_equipment[eq_partnership]'
    	);
    	...
    	$conf['field_define']=array(
			'sm_equipment[eq_sort]'=>$GLOBALS['m_eq']['text']['eq_sort'],
			'sm_equipment[eq_type]'=>$GLOBALS['m_eq']['text']['eq_type'],
			'sm_equipment[eq_partnership]'=>$GLOBALS['m_eq']['text']['eq_partnership'],
    	);
    	
    	$conf['table_form']=array(
			'sm_equipment'=>$this->table_form
    	);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_edit']=array(
			...
			'content[eq_sort]',
			'content[eq_unit]',
			'content[eq_maintenance]',
			'content[eq_description]',
			'content[eq_parameter]',
			'content[eq_price]',
			'content[eq_org]',
			'content[eq_org_s]',
			'content[eq_partnership]',
		);
		...
		$data_out['json_field_define']['eq_sort']=get_html_json_for_arr($GLOBALS['m_eq']['text']['eq_sort']);
		$data_out['json_field_define']['eq_type']=get_html_json_for_arr($GLOBALS['m_eq']['text']['eq_type']);
		$data_out['json_field_define']['eq_partnership']=get_html_json_for_arr($GLOBALS['m_eq']['text']['eq_partnership']);
		/************数据读取*****************/
		...
		switch ($data_get['act']) {
			...
			case STAT_ACT_VIEW:
				try {
					...
					if($data_db['content']['eq_org']){
						$data_db['content']['eq_org_s']=$this->m_base->get_field_where('sys_org','o_name',"AND o_id = '{$data_db['content']['eq_org']}'");
					}
					}
				} catch (Exception $e) {
				}
