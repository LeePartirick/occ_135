 ...
 * 配置类
 */
class M_proc_eq extends CI_Model {
	
	private $acl_list;
	private $p_id='PROC_EQ';
	...
	public function m_define()
    {
    	...
    	$GLOBALS[$p_id]['text']['d_acl'] = array(
			ACL_PROC_EQ_SUPER=>'超级',
			ACL_PROC_EQ_ACL=>'权限分配',
			ACL_PROC_EQ_USER=>'操作',
		);
    ...
    {
    	$this->load->model($this->p_id.'/m_eq');
    	$this->m_eq->create_import_xlsx();
    }
}