 ...
 * 配置类
 */
class M_proc_bal extends CI_Model {
	
	private $acl_list;
	private $p_id='PROC_BAL';
	...
	public function m_define()
    {
    	...
    	$GLOBALS[$p_id]['text']['d_acl'] = array(
			ACL_PROC_BAL_SUPER=>'超级',
			ACL_PROC_BAL_ACL=>'权限分配',
			ACL_PROC_BAL_USER=>'操作',
			ACL_PROC_BAL_FCHECK=>'复核',
			ACL_PROC_BAL_CHECK=>'审核',
			ACL_PROC_BAL_POSTED=>'过账',
			ACL_PROC_BAL_FINAL=>'审批',
		);
    ...
    {
//    	$this->load->model($this->p_id.'/m_bal');
//        $this->m_bal->create_import_xlsx();
    }
}