 ...
 * 配置类
 */
class M_proc_bs extends CI_Model {
	
	private $acl_list;
	private $p_id='PROC_BS';
	...
	public function m_define()
    {
    	...
    	$GLOBALS[$p_id]['text']['d_acl'] = array(
			ACL_PROC_BS_SUPER=>'超级',
			ACL_PROC_BS_ACL=>'权限分配',
			ACL_PROC_BS_USER=>'操作',
		);
    ...
    {
    	$this->load->model($this->p_id.'/m_bs');
        $this->m_bs->create_import_xlsx();
    }
}