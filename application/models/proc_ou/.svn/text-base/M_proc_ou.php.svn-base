 ...
 * 配置类
 */
class M_proc_ou extends CI_Model {
	
	private $acl_list;
	private $p_id='PROC_OU';
	...
	public function m_define()
    {
    	...
    	$GLOBALS[$p_id]['text']['d_acl'] = array(
			ACL_PROC_OU_SUPER=>'超级',
			ACL_PROC_OU_ACL=>'权限分配',
			ACL_PROC_OU_USER=>'操作',
		);
    ...
    {
    	$this->load->model($this->p_id.'/m_ou');
    	$this->m_ou->create_import_xlsx();
    }
}