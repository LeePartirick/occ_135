 ...
 *  
    发票类型
 */
class M_invoice_type extends CI_Model {
	
	//@todo 主表配置
	private $table_name='fm_invoice_type';
	private $pk_id='it_id';
	private $table_form;
	private $title='发票类型';
	private $model_name = 'm_invoice_type';
	private $url_conf = 'proc_bud/invoice_type/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_INVOIVE_TYPE') ) return;
    	define('LOAD_M_INVOIVE_TYPE', 1);
    	
    	//define
    	//it_field
		define('sum', 'sum');//无
		define('takings', 'takings');//收取管理费
		
		$GLOBALS['m_invoice_type']['text']['field_count'] = array(
			'sum'=>'发票金额',
			'takings'=>'营业收入',
		);
    }
	...
	public function del($id)
    {
		...
		/************数据处理*****************/
    	$arr_link=array(
			'fm_bill.bill_category',
			'fm_tax_type.t_invoice_type'
		);
		...
		if(count($arr_link) > 0)
		{
			foreach ($arr_link as $v ) {
				$arr_tmp = explode('.', $v);
				$field=$this->m_base->get_field_where($arr_tmp[0],$arr_tmp[1]," AND {$arr_tmp[1]} LIKE '%{$id}%' ");
				if($field)
				{
					$rtn['rtn'] = FALSE;
	...
	public function create_import_xlsx()
    {
    	...
    	$conf['field_edit']=array(
    		'fm_invoice_type[it_name]',
			'fm_invoice_type[it_taking_count]',
			'fm_invoice_type[it_tax_count]',
			'fm_invoice_type[it_note]',
    	);
    	...
    	$conf['table_form']=array(
    		'fm_invoice_type'=>$this->table_form
    	);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[it_name]',
			'content[it_taking_count]',
			'content[it_tax_count]',
		);
		...
		$data_out['json_field_define']['field_count']=get_html_json_for_arr($GLOBALS['m_invoice_type']['text']['field_count']);
		/************数据读取*****************/
			...
			case STAT_ACT_VIEW:
				try {
								...
								if( $v != element($k,$data_db['content']) )
								{
									switch ($k)
									{
										case 'it_no_new':
											...
											$data_db['content'][$k] = $GLOBALS['m_base']['text']['base_yn_0'][element($k,$data_db['content'],0)];
											
											$data_out['log']['content['.$k.']']='变更前:'.element($k,$data_db['content']);
											$data_db['content'][$k] =$v ;
											
											break;
										case 'it_taking_count':
										case 'it_tax_count':
											...
											if(count($arr_tmp)>0)
											{
												...
												foreach ($arr_tmp as $v1) {
													...
													if(isset($GLOBALS['m_invoice_type']['text']['field_count'][$v1]))
													$v1 = $GLOBALS['m_invoice_type']['text']['field_count'][$v1];
													
													$data_db['content'][$k].=$v1;
												}
											}
					...
					else 
					{
						...
						{
							$msg= '发票类型【'.element($this->pk_id,$data_get).'】不存在！';
		...
		switch ($btn)
		{
			case 'save':
                ...
                //验证唯一
                if( ! empty(element('it_name',$data_post['content'])) )
                {
                    $where_check=' AND it_id != \''.element('it_id',$data_db['content']).'\'';

                    $check=$this->m_check->unique('fm_invoice_type','it_name',element('it_name',$data_post['content']),$where_check);
                    if( ! $check )
                    {
                        $rtn['err']['content[it_name]']='发票类型'.element('it_name',$data_post['content']).'已存在，不可重复创建！';
                        $check_data=FALSE;
                    }
                }
