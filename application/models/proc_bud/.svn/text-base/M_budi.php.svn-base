 ...
 *  
    预算表模板明细
 */
class M_budi extends CI_Model {
	
	//@todo 主表配置
	private $table_name='fm_bud_item';
	private $pk_id='budi_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='预算表模型明细';
	private $model_name = 'm_budi';
	private $url_conf = 'proc_bud/budi/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_BUDI') ) return;
    	define('LOAD_M_BUDI', 1);
    	
    	//define
    	
    	//CSS
		$GLOBALS['m_budi']['text']['budi_css'] = array(
			'font-weight:bold'=>'粗体',
			'background-color:#FFFFE7'=>'背景色-黄',
			'background-color:#E8EEF7'=>'背景色-蓝',
		);
    }
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_edit']=array(
			...
			'content[budi_sum_edit]',
			'content[budi_sum_notax_empty]',
			'content[budi_sub_check]',
			'content[budi_ou_show]',
    		'content[budi_ou]',
    		'content[budi_ou_s]',
    		'content[budi_sn]',
		...
		$data_out['json_field_define']['budi_css'] = get_html_json_for_arr($GLOBALS['m_budi']['text']['budi_css']);
		/************数据读取*****************/
				...
				try {
					...
					else 
					{
						...
						if(  element('fun_no_db', $data_get) )
						{
							...
							$data_db['content']['budi_sub_s']=$this->m_base->get_field_where('fm_subject','sub_name'," AND sub_id = '{$data_db['content']['budi_sub']}'");
							$data_db['content']['budi_css']=explode(';', $data_db['content']['budi_css']);
							break;
						}
		...
		switch ($data_get['act']) {
			case STAT_ACT_CREATE:
				...
				//创建默认值
				$data_db['content']['budi_sum_edit']=1;
				$data_db['content']['budi_rate']=100;
...
//			$data_out['op_disable'][] = 'content[budi_sn]';
			$data_out['op_disable'][] = 'content[budi_sub]';
			$data_out['op_disable'][] = 'content[budi_name]';
		...
		switch ($btn)
		{
			case 'save':
				...
				if( isset($data_post['content']['budi_name'])
				 && empty($data_post['content']['budi_name']) )
				{
					$data_post['content']['budi_name'] = element('budi_sub_s', $data_post['content']);
				}
				...
				if( element('budi_css', $data_save['content']))
				{
					$data_save['content']['budi_css']=implode(';', $data_save['content']['budi_css']);
				}
				/************事件处理*****************/
				switch ($data_get['act']) {
					case STAT_ACT_CREATE:
