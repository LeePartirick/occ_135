 ...
 *  
    工作经历
 */
class M_oa_offai extends CI_Model {
	
	//@todo 主表配置
	private $table_name='oa_offa_item';
	private $pk_id='offai_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='信息系统';
	private $model_name = 'm_oa_offai';
	private $url_conf = 'proc_office/oa_offai/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_OA_OFFAI') ) return;
    	define('LOAD_M_OA_OFFAI', 1);
	...
	public function del_no_kt($id)
    {
		...
		$sql="DELETE FROM oa_offa_item 
		WHERE offai_c_id = '{$id}' AND (offai_person_start = '' OR offai_person_start IS NULL) ";
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[offai_offi_id]',
			'content[offai_model]',
			'content[offai_person_start_s]',
			'content[offai_time_start]',
			'content[offai_c_id]'
		);
		...
		$data_out['field_edit']=array(
			'content[offai_offi_id]',
			'content[offai_offi_id_s]',
			'content[offai_person_start]',
			'content[offai_person_start_s]',
			'content[offai_time_start]',
			'content[offai_model]',
			'content[offai_person_end]',
			'content[offai_person_end_s]',
			'content[offai_time_end]',
			'content[offai_note]',
			'content[offai_flag_alert]',
			'content[offai_c_id]',
			'content[c_org]'
		);
			...
			case STAT_ACT_VIEW:
				try {
					...
					else 
					{
						...
						if($data_db['content']['offai_offi_id'])
							$data_db['content']['offai_offi_id_s'] = $this->m_base->get_field_where('oa_office','offi_name'," AND offi_id = '{$data_db['content']['offai_offi_id']}'");

						if($data_db['content']['offai_person_end'])
							$data_db['content']['offai_person_end_s'] = $this->m_base->get_c_show_by_cid($data_db['content']['offai_person_end']);

						if($data_db['content']['offai_person_start'])
							$data_db['content']['offai_person_start_s'] = $this->m_base->get_c_show_by_cid($data_db['content']['offai_person_start']);
							
						$data_db['content']['c_org'] = $this->m_base->get_field_where('sys_contact','c_org',"AND c_id = '{$data_db['content']['offai_c_id']}'");
		...
		switch ($data_get['act']) {
			case STAT_ACT_CREATE:
				...
				//创建默认值
				//$data_db['content']['offai_flag_alert']=1;
				$data_db['content']['offai_person_start'] = $this->sess->userdata('c_id');
				$data_db['content']['offai_person_start_s'] = $this->sess->userdata('c_name').'['.$this->sess->userdata('a_login_id').']';
				$data_db['content']['offai_time_start'] = date('Y-m-d');
