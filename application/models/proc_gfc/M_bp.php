...
class M_bp extends CI_Model {
	...
	//@todo 主表配置
	private $table_name='pm_bill_plan';
	private $pk_id='bp_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='开票回款计划';
	private $model_name = 'm_bp';
	private $url_conf = 'proc_gfc/bp/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_BP') ) return;
    	define('LOAD_M_BP', 1);
    	
    	//define
    	
    	// 货款性质类型值定义
		DEFINE('BP_TYPE_YFK', '1');// 预付款
		DEFINE('BP_TYPE_JDK', '2');// 进度款
		DEFINE('BP_TYPE_YSK', '3');// 验收款
		DEFINE('BP_TYPE_ZBJ', '4');// 质保款
		DEFINE('BP_TYPE_QEK', '5');// 全额款
		DEFINE('BP_TYPE_QT', '6');// 其他
		$GLOBALS['m_bp']['text']['bp_type'] = array(
			BP_TYPE_YFK => '预付款',
			BP_TYPE_JDK => '进度款',
			BP_TYPE_YSK => '验收款',
			BP_TYPE_ZBJ => '质保款',
	...
	public function create_import_xlsx()
    {
    	...
    	$conf['field_edit']=array(
			'pm_bill_plan[bp_time]',
			'pm_bill_plan[bp_note]',
    	);
