 ...
 *  
    职位
 */
class M_hr_job extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_job';
	private $pk_id='job_id';
	private $table_form;
	private $title='职位';
	private $model_name = 'm_hr_job';
	private $url_conf = 'proc_hr/hr_job/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_JOB') ) return;
    	define('LOAD_M_HR_JOB', 1);
	...
	public function get_code($data_save=array())
    {
    	...
    	$pre='JOB'.date("Ym");
    	$where .= " AND job_code LIKE  '{$pre}%'"; 
    	
    	$max_code=$this->m_db->get_m_value('hr_job','job_code',$where);
		...
		switch ($btn)
		{
			case 'save':
				...
				if($btn == 'save')
				{
					...
					//验证唯一
					if( ! empty(element('job_name',$data_post['content'])) )
					{
						$where_check=' AND job_id != \''.element('job_id',$data_db['content']).'\'';
								
						$check=$this->m_check->unique('hr_job','job_name',element('job_name',$data_post['content']),$where_check);
						if( ! $check )
						{
							$rtn['err']['content[job_name]']='职位名称【'.element('job_name',$data_post['content']).'】已存在，不可重复创建！';
							$check_data=FALSE;
						}
					}
				...
				switch ($data_get['act']) {
					case STAT_ACT_CREATE:
						...
						$data_save['content']['job_code']=$this->get_code($data_save['content']);
