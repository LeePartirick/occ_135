 ...
 *  
    工作经历
 */
class M_hr_work extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_work';
	private $pk_id='hr_w_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='工作经历';
	private $model_name = 'm_hr_work';
	private $url_conf = 'proc_hr/hr_work/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_WORK') ) return;
    	define('LOAD_M_HR_WORK', 1);
    	
    	//define
		define('HR_W_TYPE_QZ', 1);

		$GLOBALS['m_hr_work']['text']['hr_w_type']=array(
			HR_W_TYPE_QZ=>'全职',
		);
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[hr_w_time_start]',
			'content[hr_w_time_end]',
			'content[hr_w_org]',
			'content[hr_w_type]',
			'content[hr_w_job]',
			'content[hr_w_person]',
			'content[hr_w_person_tel]',
			'content[hr_w_salary]'
		);
		...
		$data_out['json_field_define']['hr_w_type']=get_html_json_for_arr($GLOBALS['m_hr_work']['text']['hr_w_type']);
		/************数据读取*****************/
								...
								if( $v != element($k,$data_db['content']) )
								{
									switch ($k)
									{
										case 'hr_w_type':
											...
											$data_out['log']['content['.$k.']']='变更前:'.$GLOBALS['m_hr_work']['text']['hr_w_type'][element($k,$data_db['content'])];
											$data_db['content'][$k] =$v ;
		...
		switch ($data_get['act']) {
			case STAT_ACT_CREATE:
				...
				//创建默认值
				$data_db['content']['hr_w_type'] = HR_W_TYPE_QZ;
				//个性化配置
	...
	public function show_change_hr_info($id,$field,$v)
	{
		...
		switch ($field)
		{
			...
			case 'hr_w_time_end':
				...
				$rtn['field'] = 'hr_w_time_start';
				$rtn['err_msg'] = $v['hr_w_time_start'].' 至 '.$v['hr_w_time_end'];
				
				break;

			case 'hr_w_person':
			case 'hr_w_person_tel':

				$rtn['field'] = 'hr_w_person';
				$rtn['err_msg'] = $v['hr_w_person'].','.$v['hr_w_person_tel'];

				break;

			case 'hr_w_type':

				$rtn['err_msg'] = $GLOBALS['m_hr_work']['text']['hr_w_type'][$v['hr_w_type']];
