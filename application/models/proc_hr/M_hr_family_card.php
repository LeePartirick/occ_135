 ...
 *  
    培训记录
 */
class M_hr_family_card extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_family_card';
	private $pk_id='hr_fcard_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='培训记录';
	private $model_name = 'm_hr_family_card';
	private $url_conf = 'proc_hr/hr_family_card/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_FAMILY_CARD') ) return;
    	define('LOAD_M_HR_FAMILY_CARD', 1);
    	
    	//define
    	
    	define('HR_FCARD_TYPE_JJZ', 1);
		define('HR_FCARD_TYPE_LK', 2);
		$GLOBALS['m_hr_family_card']['text']['hr_fcard_type'] = array(
			HR_FCARD_TYPE_JJZ => '港澳台居住证',
			HR_FCARD_TYPE_LK => '绿卡',
		);
    }
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[hr_fcard_type]',
			'content[hr_fcard_name]',
			'content[hr_fcard_code]',
			'content[hr_fcard_person]',
		);
		...
		$data_out['json_field_define']['hr_fcard_type']=get_html_json_for_arr($GLOBALS['m_hr_family_card']['text']['hr_fcard_type']);
		/************数据读取*****************/
		...
		switch ($data_get['act']) {
			case STAT_ACT_CREATE:
				...
				//创建默认值
				$url=substr(element('fun_no_db',$this->uri->uri_to_assoc(4)),16,-10);
				if($url == 'hr_family_card_gat'){
					$data_db['content']['hr_fcard_type'] = HR_FCARD_TYPE_JJZ;
				}else{
					$data_db['content']['hr_fcard_type'] = HR_FCARD_TYPE_LK;
				}
				//个性化配置
	...
	public function show_change_hr_info($id,$field,$v)
	{
		...
		switch ($field)
		{
			...
			case 'hr_fcard_type':
				...
				$rtn['err_msg'] = $GLOBALS['m_hr_family_card']['text']['hr_fcard_type'][$v['hr_fcard_type']];
