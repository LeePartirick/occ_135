 ...
 *  
    家庭信息
 */
class M_hr_family extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_family';
	private $pk_id='hr_f_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='家庭信息';
	private $model_name = 'm_hr_family';
	private $url_conf = 'proc_hr/hr_family/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_FAMILY') ) return;
    	define('LOAD_M_HR_FAMILY', 1);
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[hr_f_person]',
			'content[hr_f_relation]',
			'content[hr_f_nationality]',
			'content[hr_f_birthday]',
			'content[hr_f_org]',
			'content[hr_f_city]',
			'content[hr_f_job]',
			'content[hr_f_tel]',
