 ...
 *  
    身份证件
 */
class M_hr_idcard extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_idcard';
	private $pk_id='hr_idc_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='身份证件';
	private $model_name = 'm_hr_idcard';
	private $url_conf = 'proc_hr/hr_idcard/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_IDCARD') ) return;
    	define('LOAD_M_HR_IDCARD', 1);
    	
		define('HR_IDC_TYPE_HZ',1);
		define('HR_IDC_TYPE_LK',2);
		define('HR_IDC_TYPE_SFZ',3);
		define('HR_IDC_TYPE_OTHER',4);

		$GLOBALS['m_hr_idcard']['text']['hr_idc_type']=array(
			HR_IDC_TYPE_HZ=>'护照',
			HR_IDC_TYPE_LK=>'绿卡',
			HR_IDC_TYPE_SFZ=>'身份证',
			HR_IDC_TYPE_OTHER=>'其他',
		);
    }
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['json_field_define']['hr_idc_type']=get_html_json_for_arr($GLOBALS['m_hr_idcard']['text']['hr_idc_type']);
		/************数据读取*****************/
								...
								if( $v != element($k,$data_db['content']) )
								{
									switch ($k)
									{
										case 'hr_idc_type':
											...
											$data_out['log']['content['.$k.']']='变更前:'.$GLOBALS['m_hr_idcard']['text']['hr_idc_type'][element($k,$data_db['content'])];
	...
	public function show_change_hr_info($id,$field,$v)
	{
		...
		switch ($field)
		{
			case 'hr_idc_type':
				...
				$rtn['err_msg'] = $GLOBALS['m_hr_idcard']['text']['hr_idc_type'][$v['hr_idc_type']];
