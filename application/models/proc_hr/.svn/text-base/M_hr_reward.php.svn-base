 ...
 *  
    奖惩记录
 */
class M_hr_reward extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_reward';
	private $pk_id='hr_rp_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='奖惩记录';
	private $model_name = 'm_hr_reward';
	private $url_conf = 'proc_hr/hr_reward/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_REWARD') ) return;
    	define('LOAD_M_HR_REWARD', 1);
    	
    	//define
		define('HR_RP_TYPE_JIANG', 1);
		define('HR_RP_TYPE_CHENG', 2);

		$GLOBALS['m_hr_reward']['text']['hr_rp_type']=array(
			HR_RP_TYPE_JIANG=>'奖',
			HR_RP_TYPE_CHENG=>'罚',
		);
    }
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			...
			'content[hr_rp_type]',
			'content[hr_rp_cause]',
			'content[hr_rp_ou_approve]',
			'content[hr_rp_ou_approve_s]',
			'content[hr_rp_person_approve]',
			'content[hr_rp_person_approve_s]',
			'content[hr_rp_doc_code]',
			'content[hr_rp_content]',
		);
		...
		$data_out['json_field_define']['hr_rp_type']=get_html_json_for_arr($GLOBALS['m_hr_reward']['text']['hr_rp_type']);
		/************数据读取*****************/
				...
				try {
						...
						if( count(element('content',$data_change))>0)
						{
							foreach (element('content',$data_change) as $k=>$v) 
							{
								if( $v != element($k,$data_db['content']) )
								{
									...
									{
										case 'hr_rp_type':
											$data_out['log']['content['.$k.']']='变更前:'.$GLOBALS['m_hr_reward']['text']['hr_rp_type'][element($k,$data_db['content'])];
					...
					else 
					{
						...
						if( empty($data_db['content'][$this->pk_id]) )
						{
							...
							redirect('base/main/show_err/msg/'.fun_urlencode($msg));
							if($data_db['content']['hr_rp_person_approve'])
								$data_db['content']['hr_rp_person_approve_s']=$this->m_base->get_field_where('sys_contact','c_name',"AND c_id = '{$data_db['content']['hr_rp_person_approve']}'");
						}

						if($data_db['content']['hr_rp_ou_approve'])
							$data_db['content']['hr_rp_ou_approve_s']=$this->m_base->get_field_where('sys_ou','ou_name',"AND ou_id = '{$data_db['content']['hr_rp_ou_approve']}'");
						if($data_db['content']['hr_rp_person_approve'])
							$data_db['content']['hr_rp_person_approve_s']=$this->m_base->get_field_where('sys_contact','c_name',"AND c_id = '{$data_db['content']['hr_rp_person_approve']}'");
					}
				} catch (Exception $e) {
	    ...
	    $data_out['c_org'] = element('c_org', $data_db['content']);
	...
	public function show_change_hr_info($id,$field,$v)
	{
		...
		switch ($field)
		{
			case 'hr_rp_type':
				...
				$rtn['err_msg'] = $GLOBALS['m_hr_reward']['text']['hr_rp_type'][$v['hr_rp_type']];
