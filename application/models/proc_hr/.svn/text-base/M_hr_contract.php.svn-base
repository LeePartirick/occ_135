 ...
 *  
    合同
 */
class M_hr_contract extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_contract';
	private $pk_id='cont_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='合同';
	private $model_name = 'm_hr_contract';
	private $url_conf = 'proc_hr/hr_contract/edit';
...
//        $this->load->model('proc_hr/m_hr_info');
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_CONTRACT') ) return;
    	define('LOAD_M_HR_CONTRACT', 1);
	...
	public function create_import_xlsx()
    {
    	...
    	$conf['field_edit']=array(
			'hr_contract[c_id]',
			'hr_contract[cont_year]',
			'hr_contract[cont_hr_type]',
			'hr_contract[cont_hr_type_work]',
			'hr_contract[cont_time_start]',
			'hr_contract[cont_time_end]',
			'hr_contract[cont_type]',
    	);
    	...
    	$conf['field_define']=array(
			'hr_contract[cont_hr_type]'=>$GLOBALS['m_hr']['text']['hr_type'],
			'hr_contract[cont_hr_type_work]'=>$GLOBALS['m_hr']['text']['hr_type_work'],
			'hr_contract[cont_type]'=>$GLOBALS['m_hr']['text']['cont_type'],
    	);
    	
    	$conf['table_form']=array(
			'hr_contract'=>$this->table_form
    	);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[c_id]',
			'content[cont_hr_type]',
			'content[cont_hr_type_work]',
			'content[cont_type]',
			'content[cont_time_start]',
			'content[cont_time_end]',
		);
...
//            'content[cont_year]'
		...
		$data_out['json_field_define']['cont_hr_type']=get_html_json_for_arr($GLOBALS['m_hr']['text']['hr_type']);
		$data_out['json_field_define']['cont_hr_type_work']=get_html_json_for_arr($GLOBALS['m_hr']['text']['hr_type_work']);
		$data_out['json_field_define']['cont_type']=get_html_json_for_arr($GLOBALS['m_hr']['text']['cont_type']);
		/************数据读取*****************/
			...
			case STAT_ACT_VIEW:
				try {
					...
					else 
					{
						...
						$data_db['content_hr'] =  $this->m_table_op->get($data_db['content']['c_id']);

						$data_db['content'] = array_merge($data_db['content_c'],$data_db['content_hr'],$data_db['content']);
						...
						$data_db['content']['c_name_s'] = $this->m_base->get_c_show_by_cid($data_db['content']['c_id']);
		...
		switch ($data_get['act']) {
			case STAT_ACT_CREATE:
				...
				//创建默认值
				$data_db['content']['cont_year']=($this->input->get('cont_time_end')-$this->input->get('cont_time_start'));
				//个性化配置
				...
				if( element('c_id', $data_db['content']) )
				{
					$data_db['content']['c_name_s'] = $this->m_base->get_c_show_by_cid($data_db['content']['c_id']);
                    ...
                    if(!empty(element('c_id',$data_db['content']))) {
                        //人员类别
                        $data_db['content']['cont_hr_type'] = $this->m_base->get_field_where('hr_info', 'hr_type', " AND c_id='{$data_db['content']['c_id']}'");
                        //用工形式
                        $data_db['content']['cont_hr_type_work'] = $this->m_base->get_field_where('hr_info', 'hr_type_work', " AND c_id='{$data_db['content']['c_id']}'");
                    }
				}
		...
		switch ($btn)
		{
			case 'save':
				...
				if($btn == 'save')
				{
					...
					//校验时间
					if( ! empty(element('cont_time_start',$data_post['content'])) && ! empty(element('cont_time_end',$data_post['content']))){

						if(element('cont_time_start',$data_post['content']) >= element('cont_time_end',$data_post['content'])){
							$rtn['err']['content[cont_time_start]']='时间选择不合法！';
							$rtn['err']['content[cont_time_end]']='时间选择不合法！';
							$check_data=FALSE;
						}
					}
    ...
    public function show_change_hr_info($id,$field,$v)
    {
        $rtn=array();
        $rtn['id'] = $id;
        $rtn['field'] = $field;
