...
职位
 */
class M_hr_tec extends CI_Model {

	//@todo 主表配置
	private $table_name='hr_tec';
	private $pk_id='tec_id';
	private $table_form;
	private $title='技术方向';
	private $model_name = 'm_hr_tec';
	private $url_conf = 'proc_hr/hr_tec/edit';
	...
	{
		//@todo 定义
		if( defined('LOAD_M_HR_TEC') ) return;
		define('LOAD_M_HR_TEC', 1);
		...
		switch ($btn)
		{
			case 'save':
				...
				if($btn == 'save')
				{
					...
					//验证唯一
					if( ! empty(element('tec_name',$data_post['content'])) )
					{
						$where_check=' AND tec_id != \''.element('tec_id',$data_db['content']).'\'';

						$check=$this->m_check->unique('hr_tec','tec_name',element('tec_name',$data_post['content']),$where_check);
						if( ! $check )
						{
							$rtn['err']['content[tec_name]']='技术方向【'.element('tec_name',$data_post['content']).'】已存在，不可重复创建！';
							$check_data=FALSE;
						}
					}
