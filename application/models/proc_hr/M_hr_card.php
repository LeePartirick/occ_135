 ...
 *  
    卡帐信息
 */
class M_hr_card extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_card';
	private $pk_id='hr_card_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='卡帐信息';
	private $model_name = 'm_hr_card';
	private $url_conf = 'proc_hr/hr_card/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_CARD') ) return;
    	define('LOAD_M_HR_CARD', 1);
    ...
    public function create_import_xlsx()
    {
        ...
        $conf['field_edit']=array(
            ...
            'hr_card[hr_card_type]',
            'hr_card[hr_card_code]',
            'hr_card[hr_card_note]',
        );
        ...
        $conf['field_define']=array(
            'hr_card[hr_card_type]'=>$GLOBALS['m_hr']['text']['c_bank_type'],
        );
        
        $this->arr_table_form['sys_contact']['fields']['c_login_id']['comment']='关联账号';
        $conf['table_form']=array(
            'sys_contact'=>$this->arr_table_form['sys_contact'],
//            'hr_info'=>$this->arr_table_form['hr_info'],
            'hr_card'=>$this->table_form,
        );
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[hr_card_type]',
			'content[hr_card_code]',
			'content[c_id]',
		);
		...
		$data_out['json_field_define']['hr_card_type']=get_html_json_for_arr($GLOBALS['m_hr']['text']['c_bank_type']);
		/************数据读取*****************/
								...
								if( $v != element($k,$data_db['content']) )
								{
									switch ($k)
									{
										case 'hr_card_type':
											...
											$data_out['log']['content['.$k.']']='变更前:'.$GLOBALS['m_hr']['text']['c_bank_type'][element($k,$data_db['content'])];
		...
		switch ($btn)
		{
			case 'save':
				...
				if($btn == 'save')
				{
					...
					if($check_data)
					{
						switch( element('hr_card_type', $data_post['content']))
						{
							case C_BANK_TYPE_SHEBAO:
							case C_BANK_TYPE_GJJ:
								
								$where_check=' AND hr_card_id != \''.element('hr_card_id',$data_db['content']).'\'  AND hr_card_type = \''.element('hr_card_type',$data_post['content']).'\' ';
			
								$check=$this->m_check->unique('hr_card','hr_card_code',element('hr_card_code',$data_post['content']),$where_check);
			
								if( ! $check )
								{
									$rtn['err']['content[hr_card_type]']=''.$GLOBALS['m_hr']['text']['c_bank_type'][$data_post['content']['hr_card_type']]
									.'【'.$data_post['content']['hr_card_code'].'】已存在，不可重复创建！';
									$check_data=FALSE;
								}
								else
								{
									$where_check= ' AND c_id = \''.element('c_id',$data_db['content']).'\' AND hr_card_id != \''.element('hr_card_id',$data_db['content']).'\' ';
									$check=$this->m_check->unique('hr_card','hr_card_type',element('hr_card_type',$data_post['content']),$where_check);
									
									if( ! $check )
									{
										$rtn['err']['content[hr_card_type]']='【'.$this->m_base->get_c_show_by_cid(element('c_id',$data_db['content'])).'】'
										.'的【'.$GLOBALS['m_hr']['text']['c_bank_type'][$data_post['content']['hr_card_type']].'】已存在，不可重复创建！';
										$check_data=FALSE;
									}
								}
	...
	public function show_change_hr_info($id,$field,$v)
	{
		...
		switch ($field)
		{
			case 'hr_card_type':
				...
				$rtn['err_msg'] = $GLOBALS['m_hr']['text']['c_bank_type'][$v['hr_card_type']];
