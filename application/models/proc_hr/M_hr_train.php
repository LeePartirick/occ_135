 ...
 *  
    培训记录
 */
class M_hr_train extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_train';
	private $pk_id='hr_t_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='培训记录';
	private $model_name = 'm_hr_train';
	private $url_conf = 'proc_hr/hr_train/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_TRAIN') ) return;
    	define('LOAD_M_HR_TRAIN', 1);
    	
    	//define
    	
    	define('HR_T_TYPE_ORG', 1);
    }
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[hr_t_time_start]',
			'content[hr_t_time_end]',
			'content[hr_t_name]',
			'content[hr_t_org]',
			'content[hr_t_ca_name]',
		);
		...
		$data_out['field_edit']=array(
			...
			'content[hr_t_org]',
			'content[hr_t_ca_name]',
			'content[hr_t_time_ca_end]',
			'content[hr_t_type_last]',
			'content[hr_t_person]',
			'content[hr_t_person_tel]',
			'content[hr_t_expense]',
			'content[hr_t_date_work]',
			'content[hr_t_note]',
			'content[hr_t_type]',
			'content[c_id]',
		);
		...
		if(element('hr_t_type', $data_db['content']) != HR_T_TYPE_ORG)
		{
			$data_out['field_required'][]='content[hr_t_person]';
			$data_out['field_required'][]='content[hr_t_person_tel]';

			$data_db['content']['hr_t_type_last'] = BASE_N;

			$data_out['op_disable'][]='content[hr_t_expense]';
			$data_out['op_disable'][]='content[hr_t_date_work]';
		...
		{
			$data_out['op_disable'][]='content[hr_t_type_last]';
			$data_out['op_disable'][]='content[hr_t_person]';
			$data_out['op_disable'][]='content[hr_t_person_tel]';

			$data_out['field_required'][]='content[hr_t_expense]';
			$data_out['field_required'][]='content[hr_t_date_work]';
		}
		/************事件处理*****************/
	...
	public function show_change_hr_info($id,$field,$v)
	{
		...
		switch ($field)
		{
			...
			case 'hr_t_time_end':
				...
				$rtn['field'] = 'hr_t_time_start';
				$rtn['err_msg'] = $v['hr_t_time_start'].' 至 '.$v['hr_t_time_end'];
				
				break;
				
			case 'hr_t_person':
			case 'hr_t_person_tel':
				
				$rtn['field'] = 'hr_t_person';
				$rtn['err_msg'] = $v['hr_t_person'].','.$v['hr_t_person_tel'];
				
				break;
				
			case 'hr_t_type_last':
				
				$rtn['err_msg'] = $GLOBALS['m_base']['text']['base_yn'][$v['hr_t_type_last']];
