 ...
 *  
    教育信息
 */
class M_hr_edu extends CI_Model {
	
	//@todo 主表配置
	private $table_name='hr_edu';
	private $pk_id='hr_edu_id';
	private $table_form;
	private $arr_table_form=array();
	private $title='教育信息';
	private $model_name = 'm_hr_edu';
	private $url_conf = 'proc_hr/hr_edu/edit';
    ...
    {
    	//@todo 定义
    	if( defined('LOAD_M_HR_EDU') ) return;
    	define('LOAD_M_HR_EDU', 1);
    	
		$this->load->model('proc_hr/m_hr');
		
		//学习情况
		define('HR_EDU_TYPE_STUDY_GRADUATION', 1);//毕业
		define('HR_EDU_TYPE_STUDY_COMPLETION', 2);//结业
		define('HR_EDU_TYPE_STUDY_INCOMPLETE', 3);//肄业
		define('HR_EDU_TYPE_STUDY_LEARNING', 4);//在学

		$GLOBALS['m_hr_edu']['text']['hr_edu_type_study'] = array(
			HR_EDU_TYPE_STUDY_GRADUATION => '毕业',
			HR_EDU_TYPE_STUDY_COMPLETION => '结业',
			HR_EDU_TYPE_STUDY_INCOMPLETE => '肄业',
			HR_EDU_TYPE_STUDY_LEARNING => '在学',
		);
	...
	public function check_acl( $data_db=array() ,$acl_list = NULL)
    {
    	...
    	$check_acl=FALSE;
   	 	...
   	 	if( element('c_id', $data_db['content']) == $this->sess->userdata('c_id') 
   	 	 || (  $act == STAT_ACT_CREATE
   	 	 	   && element('c_id', $data_get) == $this->sess->userdata('c_id') )
   	 	 	)
        {
        	$check_acl=TRUE;
        }
...
//    	$this->m_excel->create_import_file($path,$conf);
	...
	public function load($data_get=array(),$data_post=array())
	{
		...
		$data_out['field_required']=array(
			'content[hr_edu_start_time]',
			'content[hr_edu_end_time]',
			'content[hr_edu_school]',
			'content[hr_edu_xl]',
			'content[hr_edu_major]',
			'content[hr_edu_type_day]',
			'content[hr_edu_type_study]',
			'content[hr_edu_person]',
			'content[hr_edu_person_tel]'
		);
		...
		$data_out['flag_check'] = $this->input->post('flag_check');
		/************字段定义*****************/
		//@todo 字段定义
		$arr_field=array_unique(array_merge($data_out['field_edit'], $data_out['field_view']));
		
		$data_out['json_field_define']=array();
		$data_out['json_field_define']['hr_edu_xl']=get_html_json_for_arr($GLOBALS['m_hr']['text']['c_xl']);
		$data_out['json_field_define']['base_yn']=get_html_json_for_arr($GLOBALS['m_base']['text']['base_yn']);
		$data_out['json_field_define']['hr_edu_type_study']=get_html_json_for_arr($GLOBALS['m_hr_edu']['text']['hr_edu_type_study']);
		/************数据读取*****************/
						...
						if( count(element('content',$data_change))>0)
						{
							foreach (element('content',$data_change) as $k=>$v) 
							{
								if( $v != element($k,$data_db['content']) )
								{
									...
									{
										case 'hr_edu_xl':
											$data_out['log']['content['.$k.']']='变更前:'.$GLOBALS['m_hr']['text']['c_xl'][element($k,$data_db['content'])];
										...
										case 'hr_edu_type_day':
											$data_out['log']['content['.$k.']']='变更前:'.$GLOBALS['m_base']['text']['base_yn'][element($k,$data_db['content'])];
										...
										case 'hr_edu_type_study':
											$data_out['log']['content['.$k.']']='变更前:'.$GLOBALS['m_hr_edu']['text']['hr_edu_type_study'][element($k,$data_db['content'])];
	...
	public function show_change_hr_info($id,$field,$v)
	{
		...
		switch ($field)
		{
			case 'hr_edu_start_time':
			case 'hr_edu_end_time':

				$rtn['field'] = 'hr_edu_time_start';
				$rtn['err_msg'] = $v['hr_edu_start_time'].' 至 '.$v['hr_edu_end_time'];

				break;

			case 'hr_edu_person':
			case 'hr_edu_person_tel':

				$rtn['field'] = 'hr_edu_person';
				$rtn['err_msg'] = $v['hr_edu_person'].','.$v['hr_edu_person_tel'];

				break;

			case 'hr_edu_xl':

				$rtn['err_msg'] = $GLOBALS['m_hr']['text']['c_xl'][$v['hr_edu_xl']];

				break;

			case 'hr_edu_type_day':

				$rtn['err_msg'] = $GLOBALS['m_base']['text']['base_yn'][$v['hr_edu_type_day']];

				break;

			case 'hr_edu_type_study':

				$rtn['err_msg'] = $GLOBALS['m_hr_edu']['text']['hr_edu_type_study'][$v['hr_edu_type_study']];
