...
<script type="text/javascript">
				...
				else
				{
					...
					else
					{
						...
						if('<?=$act?>'=='<?=STAT_ACT_CREATE?>')
						{
							...
							url+='/act/<?=STAT_ACT_EDIT?>/hr_t_id/'+json.id
	...
	//批准人自动补全
	function load_offai_person_start<?=$time?>()
	{
		var opt = $('#txtb_offai_person_start_s<?=$time?>').textbox('options');

		if(  opt.readonly ) return;

		$('#txtb_offai_person_start_s<?=$time?>').textbox('textbox').autocomplete({
			...
			onSelect: function (suggestion) {
				$('#txtb_offai_person_start_s<?=$time?>').textbox('setValue',base64_decode(suggestion.data.c_name));
				$('#txtb_offai_person_start<?=$time?>').val(base64_decode(suggestion.data.c_id));
			}
		});
	}
	
	function load_offai_person_end<?=$time?>()
	{
		var opt = $('#txtb_offai_person_end_s<?=$time?>').textbox('options');

		if(  opt.readonly ) return;

		$('#txtb_offai_person_end_s<?=$time?>').textbox('textbox').autocomplete({
			serviceUrl: 'base/auto/get_json_contact',
			width:'300',
			params:{
				rows:10,
			...
			onSelect: function (suggestion) {
				$('#txtb_offai_person_end_s<?=$time?>').textbox('setValue',base64_decode(suggestion.data.c_name));
				$('#txtb_offai_person_end<?=$time?>').val(base64_decode(suggestion.data.c_id));
			}
		});
	}
	
	function load_offai_offi_id<?=$time?>()
	{
		var opt = $('#txtb_offai_offi_id<?=$time?>').combobox('options');

		var url = 'proc_office/oa_office/get_json/from/combobox/field_id/offi_id/field_text/offi_name';

		if(data_<?=$time?>['content[c_org]']) url+='/search_org/'+data_<?=$time?>['content[c_org]'];
		if(data_<?=$time?>['content[offai_offi_id]']) url+='/search_offi_id/'+data_<?=$time?>['content[offai_offi_id]'];

		$('#txtb_offai_offi_id<?=$time?>').combobox('reload',url);
		
		if(  opt.readonly ) return;

		$('#txtb_offai_offi_id<?=$time?>').combobox('textbox').bind('focus',
		function(){
