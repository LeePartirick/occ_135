...
<script type="text/javascript">
	...
	function load_table_index<?=$time?>()
	{
		...
		var col=[[
			...
			{field:'c_job_s',title:'职位',width:150,halign:'center',align:'center',sortable:true,
				formatter: fun_index_formatter<?=$time?>
			},
			{field:'c_ou_2_s',title:'二级部门',width:150,halign:'center',align:'center',sortable:true,
				formatter: fun_index_formatter<?=$time?>
			},
			{field:'c_org_s',title:'所属公司',width:150,halign:'center',align:'center',sortable:true,
				formatter: fun_index_formatter<?=$time?>
			},
		...
		var fcol=[[
			{field:'c_id',title:'',checkbox:true,halign:'center',align:'center',
				formatter: fun_index_formatter<?=$time?>
			},
		...
		$('#table_index<?=$time?>').datagrid({
			...
			url:'proc_office/oa_contact/get_json.html',
			idField:'c_id',
			sort:'<?=$sort?>',
			order:'<?=$order?>',
			rownumbers:true,
...
//			nowrap:false,
	...
	function fun_index_formatter<?=$time?>(value,row,index){
		...
		switch(this.field)
		{
			case 'c_name':
				...
				var url='proc_office/oa_contact/edit/act/2/c_id/'+base64_decode(row.c_id);
	...
	{
		var url = 'proc_office/oa_contact/edit/act/'+act;
	...
	function fun_operate_more<?=$time?>(list,btn)
	{
		//保存配置
		$.ajax({
			url:"app/run_back/proc_office/oa_contact/fun_operate_more.html",
			type:"POST",
			data:{
				list : list,
				btn : btn,
