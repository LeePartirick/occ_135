		 			...
		 			<tr>
		       		...
		       		<tr>
		       			<td style="width:'50%';">
		       				...
		       				<a href="javascript:void(0)" oaname="btn_import" class="easyui-linkbutton oa_op" data-options="iconCls:'icon-xlsx'" onClick="fun_xlsx_import('proc_office/m_oa_office','信息系统','proc_office/oa_office/import')">导入</a>
		       			</td>
		       			<td style="width:'50%';text-align:right;">
		 		...
		 		<form id="f_<?=$time?>" class="easyui-form" method="post" data-options="novalidate:false" time="<?=$time?>"> 
		 			<table id="table_f_<?=$time?>" style="width:800px;margin:0 auto;">
						...
						<tr>
							...
							<td colspan="3">
								<input name="content[offi_name]"
									   oaname="content[offi_name]"
						...
						<tr>
							...
							<td colspan="3">
								<input name="content[offi_org]"
									   oaname="content[offi_org]"
									   id="txtb_offi_org<?=$time?>"
						...
						<tr>
							...
							<td colspan="3">
								<input name="content[offi_org_default]"
									   oaname="content[offi_org_default]"
									   id="txtb_offi_org_default<?=$time?>"
									   class="easyui-combotree oa_input"
									   data-options="err:err,
					    			   				 limitToList:true,
					    			   				 valueField:'id',
													 ...
													 onCheck : function(node, checked)
													 {
													 	if(checked)
													 	{
													 		var n = $('#txtb_offi_org<?=$time?>').combotree('tree').tree('find',node.id);
													 		$('#txtb_offi_org<?=$time?>').combotree('tree').tree('check',n.target)
													 	}
													 }
													 ">
							</td>
						...
						<tr>
							...
							<td colspan="3">
								<input name="content[offi_person_start]"
									   oaname="content[offi_person_start]"
									   id="txtb_offi_person_start<?=$time?>"
									   ...
									   data-options="err:err,
						    						 ...
						    						 missingMessage:'该输入项为必填项',
					    			   				 ...
					    			   				 panelWidth:'',
													 validType:['errMsg[\'#hd_err_f_<?=$time?>\']'],
													 fun_ready: 'load_offi_person_start<?=$time?>()'
													 ">
							</td>
						</tr>
						...
						<tr>
							...
							<td style="height:35px;vertical-align:text-top;"  colspan="3">
								<input name="content[offi_note]"
									   oaname="content[offi_note]"
									   class="easyui-textbox oa_input"
									   data-options="err:err,
					    			   				 height:'50',
					    			   				 width:'100%',
