...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		...
		{field:'offl_note',title:'备注',width:150,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	...
	var fcol=[[
		{field:'offl_id',title:'',checkbox:true,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'offl_code',title:'单据编号',width:150,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'offl_c_id_s',title:'系统所有人',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_office/office_logout/get_json.html',
		idField:'offl_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'offl_code':
			...
			var url='proc_office/office_logout/edit/act/2/offl_id/'+base64_decode(row.offl_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'信息系统注销-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_office/office_logout/edit/act/'+act;

	if(op_id)
		url+='/offl_id/'+op_id

	fun_index_win_open<?=$time?>('创建注销信息系统','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_office/office_logout/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
