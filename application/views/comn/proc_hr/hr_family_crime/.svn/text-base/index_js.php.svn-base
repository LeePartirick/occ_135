...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'hr_fc_description',title:'描述',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'hr_fc_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_family_crime/get_json.html',
		idField:'hr_fc_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'hr_fc_description':
			...
			var url='proc_hr/hr_family_crime/edit/act/2/hr_fc_id/'+base64_decode(row.hr_fc_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.hr_fc_description).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_hr/hr_family_crime/edit/act/'+act;
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_family_crime/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
