...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'dim_cause',title:'离职原因',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'dim_note',title:'备注',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
        {field:'ppo',title:'节点',width:200,halign:'center',align:'center',sortable:true,
            formatter: fun_index_formatter<?=$time?>
        },
	]];
	...
	var fcol=[[
				{field:'dim_id',title:'dim_id',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'dim_code',title:'单据编号',width:150,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'c_id',title:'离职人员',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'dim_date',title:'离职时间',width:120,halign:'center',align:'center',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_dim/get_json.html',
		idField:'dim_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'dim_code':
			...
			var url='proc_hr/hr_dim/edit/act/2/dim_id/'+base64_decode(row.dim_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.c_id).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
			
			break;
        case 'c_id':

            value=base64_decode(row.c_id_s);
...
{
	var url = 'proc_hr/hr_dim/edit/act/'+act;

	if(op_id)
		url+='/dim_id/'+op_id

	fun_index_win_open<?=$time?>('创建计数方向','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_dim/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
        ...
        success:function(data){
            ...
            if(json.rs)
            {
            	...
            	fun_win_sys_msg('【员工离职批量操作】操作结果:',msg)
