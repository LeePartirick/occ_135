...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'cont_year',title:'合同年限',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'cont_hr_type',title:'人员类别',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'cont_hr_type_work',title:'用工形式',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'cont_type',title:'合同性质',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]];
	...
	var fcol=[[
				{field:'cont_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'c_id',title:'姓名',width:150,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_contract/get_json.html',
		idField:'cont_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'c_id':
			...
			var url='proc_hr/hr_contract/edit/act/2/cont_id/'+base64_decode(row.cont_id);
...
{
	var url = 'proc_hr/hr_contract/edit/act/'+act;
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_contract/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
        ...
        success:function(data){
            ...
            if(json.rs)
            {
            	...
            	fun_win_sys_msg('【职务批量操作】操作结果:',msg)
