			...
			else
			{
				...
				else
				{
					...
					if('<?=$act?>'=='<?=STAT_ACT_CREATE?>')
					{
						...
						url+='/act/<?=STAT_ACT_EDIT?>/cont_id/'+json.id
...
function load_cont_year<?=$time?>(){
    var start_time=$('#start_time<?=$time?>').datebox('getValue');
    var end_time=$('#end_time<?=$time?>').datebox('getValue');
    if(start_time){
        alert(111);
    }
}
