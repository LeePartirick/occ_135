...
function load_table_index<?=$time?>()
{
	...
	var col=[[    
		{field:'c_note',title:'备注',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'job_id',title:'job_id',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'job_code',title:'编号',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'job_name',title:'职位名称',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_job/get_json.html',
		idField:'job_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'job_code':
			...
			var url='proc_hr/hr_job/edit/act/2/job_id/'+base64_decode(row.job_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'职位-'+base64_decode(row.job_name).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_hr/hr_job/edit/act/'+act;

	if(op_id)
		url+='/job_id/'+op_id

	fun_index_win_open<?=$time?>('创建职位','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_job/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
        ...
        success:function(data){
            ...
            if(json.rs)
            {
            	...
            	fun_win_sys_msg('【职位批量操作】操作结果:',msg)
