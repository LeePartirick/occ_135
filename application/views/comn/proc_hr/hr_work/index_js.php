...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'hr_w_lz_cause',title:'离职原因',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'hr_w_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_w_time_start',title:'起始时间',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_w_time_end',title:'截止时间',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_w_org',title:'公司名称',width:200,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_train/get_json.html',
		idField:'hr_w_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'hr_w_org':
			...
			var url='proc_hr/hr_train/edit/act/2/hr_w_id/'+base64_decode(row.hr_w_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.hr_w_org).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_hr/hr_train/edit/act/'+act;

	if(op_id)
		url+='/hr_w_id/'+op_id

	fun_index_win_open<?=$time?>('创建培训记录','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_train/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
