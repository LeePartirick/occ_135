			...
			else
			{
				...
				else
				{
					...
					if('<?=$act?>'=='<?=STAT_ACT_CREATE?>')
					{
						...
						url+='/act/<?=STAT_ACT_EDIT?>/hr_rp_id/'+json.id
...
//批准部门自动补全
function load_hr_rp_ou_approve<?=$time?>()
{
	var opt = $('#txtb_hr_rp_ou_approve_s<?=$time?>').textbox('options');
	...
	$('#txtb_hr_rp_ou_approve_s<?=$time?>').textbox('textbox').autocomplete({
		...
		onSelect: function (suggestion) {
			$('#txtb_hr_rp_ou_approve_s<?=$time?>').textbox('setValue',base64_decode(suggestion.data.ou_name));
			$('#txtb_hr_rp_ou_approve<?=$time?>').val(base64_decode(suggestion.data.ou_id));
		}
	});
}
//批准人自动补全
function load_hr_rp_person_approve<?=$time?>()
{
	var opt = $('#txtb_hr_rp_person_approve_s<?=$time?>').textbox('options');

	if(  opt.readonly ) return;

	$('#txtb_hr_rp_person_approve_s<?=$time?>').textbox('textbox').autocomplete({
		...
		onSelect: function (suggestion) {
			$('#txtb_hr_rp_person_approve_s<?=$time?>').textbox('setValue',base64_decode(suggestion.data.c_name));
			$('#txtb_hr_rp_person_approve<?=$time?>').val(base64_decode(suggestion.data.c_id));
		}
	});
}
</script>