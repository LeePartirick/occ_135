...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'hr_rp_cause',title:'原因',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'hr_rp_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_rp_date',title:'开始时间',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_rp_type',title:'类别',width:200,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_reward/get_json.html',
		idField:'hr_rp_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'hr_rp_doc_code':
			...
			var url='proc_hr/hr_reward/edit/act/2/hr_rp_id/'+base64_decode(row.hr_rp_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.hr_rp_type).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_hr/hr_reward/edit/act/'+act;
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_reward/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
