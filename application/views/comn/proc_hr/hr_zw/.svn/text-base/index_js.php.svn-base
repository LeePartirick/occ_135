...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'zw_note',title:'职务描述',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'zw_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'zw_name',title:'职务名称',width:300,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_zw/get_json.html',
		idField:'zw_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'zw_name':
			...
			var url='proc_hr/hr_zw/edit/act/2/zw_id/'+base64_decode(row.zw_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.zw_name).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_hr/hr_zw/edit/act/'+act;
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_zw/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
