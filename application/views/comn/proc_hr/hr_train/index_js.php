...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'hr_t_note',title:'备注',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'hr_t_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_t_time_start',title:'起始时间',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_t_time_end',title:'截止时间',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'hr_t_name',title:'培训名称',width:200,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'hr_t_name':
			...
			var url='proc_hr/hr_train/edit/act/2/hr_t_id/'+base64_decode(row.hr_t_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.hr_t_name).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
