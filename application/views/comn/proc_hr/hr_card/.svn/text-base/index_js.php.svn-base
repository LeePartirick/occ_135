...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'hr_card_code',title:'卡号',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'hr_card_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_card/get_json.html',
		idField:'hr_card_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'hr_card_code':
			...
			var url='proc_hr/hr_card/edit/act/2/hr_card_id/'+base64_decode(row.hr_card_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.hr_card_code).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_hr/hr_card/edit/act/'+act;

	if(op_id)
		url+='/hr_card_id/'+op_id
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_card/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
