...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'hr_f_person',title:'姓名',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				{field:'hr_f_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_hr/hr_family/get_json.html',
		idField:'hr_f_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'hr_f_person':
			...
			var url='proc_hr/hr_family/edit/act/2/hr_f_id/'+base64_decode(row.hr_f_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.hr_f_person).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_hr/hr_family/edit/act/'+act;
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_hr/hr_family/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
