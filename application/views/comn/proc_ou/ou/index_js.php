...
function load_table_index<?=$time?>()
{
	...
	var col=[[    
		{field:'ou_parent_s',title:'上级团队',width:200,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'ou_status',title:'状态',width:80,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'ou_class',title:'团队分类',width:100,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'ou_level',title:'组织架构',width:100,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'ou_tag',title:'标签',width:150,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'ou_org_s',title:'所属机构',width:250,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		]];
	...
	var fcol=[[
				{field:'ou_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'ou_code',title:'编号',width:100,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'ou_name',title:'团队名称',width:200,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'ou_code':
			...
			var url='proc_ou/ou/edit/act/2/ou_id/'+base64_decode(row.ou_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'团队-'+row.ou_name+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_ou/ou/edit/act/'+act;

	if(op_id)
		url+='/ou_id/'+op_id

	fun_index_win_open<?=$time?>('创建团队','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_ou/ou/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
        ...
        success:function(data){
            ...
            if(json.rs)
            {
            	...
            	fun_win_sys_msg('【团队批量操作】操作结果:',msg)
...
//上级团队查询
function load_ou_parent_search<?=$time?>()
{
