...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'eq_brand',title:'设备品牌',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'eq_model',title:'设备型号',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'eq_sort',title:'设备类别',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'eq_price',title:'参考单价',width:150,halign:'center',align:'right',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'eq_description',title:'详细描述',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'eq_org_s',title:'供应商',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]];
	...
	var fcol=[[
				{field:'eq_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'eq_name',title:'设备名称',width:100,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'eq_name':
			...
			var url='proc_eq/eq/edit/act/2/eq_id/'+base64_decode(row.eq_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.eq_name).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_eq/eq/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
...
//供应商查询
function load_eq_org_search<?=$time?>()
{
		...
		onSelect: function (suggestion) {
			...
			$('#table_search<?=$time?>').propertygrid('updateRow',{
				...
				row: {
					...
					value_s: base64_decode(suggestion.data.o_id_standard_s)
				}
			});
		}
	});
