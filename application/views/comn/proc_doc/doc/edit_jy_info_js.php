...
<script type="text/javascript">
    ...
    //借阅信息--载入
    function load_table_jy_info<?=$time?>()
    {console.log('<?=$doc_id?>')
        $('#table_jy_info<?=$time?>').datagrid({
            width:'100%',
            height:'200',
            url:'proc_doc/doc/get_json_jy/search_jy_info/'+'<?=$doc_id?>',
            singleSelect:true,
            selectOnCheck:false,
            checkOnSelect:false,
            striped:true,
            ...
            columns:[[
				{field:'docb_explain',title:'情况说明',width:200,halign:'center',align:'center',
				    formatter: fun_table_jy_info_formatter<?=$time?>,
				},
            ]],
            frozenColumns:[[
    ...
    //列格式化输出
    function fun_table_jy_info_formatter<?=$time?>(value,row,index){
        switch(this.field)
        {
        	case 'docb_code':
