...
function load_table_index<?=$time?>()
{
	...
	var col=[[
        {field:'docb_c_id_s',title:'借阅人',width:150,halign:'center',align:'center',sortable:true,
            formatter: fun_index_formatter<?=$time?>
        },
        {field:'docb_c_ou_s',title:'部门',width:150,halign:'center',align:'center',sortable:true,
            formatter: fun_index_formatter<?=$time?>
        },
        {field:'docb_type',title:'借阅（领取）',width:150,halign:'center',align:'center',sortable:true,
            formatter: fun_index_formatter<?=$time?>
        },
        {field:'ppo',title:'流程节点',width:150,halign:'center',align:'center',sortable:true,
            formatter: fun_index_formatter<?=$time?>
        }
	]];
	console.log(JSON.parse(base64_decode('<?=$columns?>')));
	if('<?=$columns?>') 
	{
	...
	var fcol=[[
				{field:'docb_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'docb_code',title:'单据编号',width:200,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},{field:'ppo',title:'流程节点',width:150,halign:'center',align:'center',sortable:true,
		            formatter: fun_index_formatter<?=$time?>
		        },
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_doc/doc_borrow/get_json.html',
		idField:'docb_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
{
	var url = 'proc_doc/doc_borrow/edit/act/'+act;
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_doc/doc_borrow/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
