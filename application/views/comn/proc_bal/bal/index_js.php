...
function load_table_index<?=$time?>()
{
	...
	var col=[[   
		{field:'bal_ou_s',title:'部门',width:120,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'bal_category_bumen',title:'是否项目',width:80,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'bal_note',title:'备注',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'bal_org_owner_s',title:'所属机构',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	...
	var fcol=[[
				{field:'bal_id',title:'bal_id',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'bal_code',title:'单据编号',width:120,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'bal_total_sum',title:'金额',width:120,halign:'center',align:'right',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'bal_time_node',title:'日期',width:100,halign:'center',align:'center',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'bal_contact_manager_s',title:'申请人',width:120,halign:'center',align:'center',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_bal/bal/get_json.html',
		idField:'bal_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'bal_code':
			...
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.bal_code)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_bal/bal/edit/act/'+act;

	if(op_id)
		url+='/bal_id/'+op_id

	fun_index_win_open<?=$time?>('创建费用报销','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_bal/bal/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
