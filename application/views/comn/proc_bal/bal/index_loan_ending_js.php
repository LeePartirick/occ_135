...
function load_table_index<?=$time?>()
{
	...
	var fcol=[[
				...
				{field:'bl_sum',title:'冲账金额',width:120,halign:'center',align:'right',sortable:true,
		            formatter: fun_index_formatter<?=$time?>,
					editor:{
						type:'numberbox',
						options:{
...
//		nowrap:false,
		...
		onLoadSuccess: function(data)
		{
			...
			$('#l_index_<?=$time?> .datagrid-header-check').find('[type="checkbox"]').hide();
		},
		onBeforeCheck: function( index, row)
		{
			if( ! row.bl_sum ) return false;
		},
		onUncheck: function( index, row)
		{
			if( row.bl_sum ) $(this).datagrid('checkRow',index);
		},
		onBeginEdit: function(index,row)
	    {
			...
			for(var i = 0;i < ed_list.length; i++)
			{
				...
				{
					case 'bl_sum':
						$(ed_list[i].target).numberbox({max: row.loan_ending_sum});
...
//申请人
function load_gfc_finance_code<?=$time?>()
{
	 var row_s =  $('#table_search<?=$time?>').propertygrid('getSelected');
	    var index_s = $('#table_search<?=$time?>').propertygrid('getRowIndex',row_s);
