...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'gfcs_note',title:'备注',width:150,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
		{field:'gfcs_id',title:'',checkbox:true,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'gfcs_name_tm',title:'脱密名称',width:200,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'gfc_name',title:'项目名称',width:200,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'gfcs_category_secret',title:'密级',width:120,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'gfcs_c_check_s',title:'审批人',width:120,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_gfc/gfc_secret/get_json.html',
		idField:'gfcs_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'gfcs_name_tm':
			...
			var url='proc_gfc/gfc_secret/edit/act/2/gfcs_id/'+base64_decode(row.gfcs_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'标密申请单-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_gfc/gfc_secret/edit/act/'+act;

	if(op_id)
		url+='/gfcs_id/'+op_id

	fun_index_win_open<?=$time?>('创建标密申请单','win',url)
}
...
//审批人查询
function load_gfcs_c_search<?=$time?>()
{
