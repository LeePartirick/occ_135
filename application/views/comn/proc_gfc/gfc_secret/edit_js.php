...
//				alert(data);
	...
	//审批人
	function load_gfcs_c_check<?=$time?>()
	{
		var opt = $('#txtb_gfcs_c_check_s<?=$time?>').textbox('options');

		if(  opt.readonly ) return;

		$('#txtb_gfcs_c_check_s<?=$time?>').textbox('textbox').autocomplete({
			serviceUrl: 'base/auto/get_json_contact/search_c_login_id/1/fun_p/proc_gfc/fun_m/m_gfc_secret/fun_f/search_gfcs_c_check',
			width:'300',
			params:{
				rows:10,
			},
			onSelect: function (suggestion) {
				$('#txtb_gfcs_c_check_s<?=$time?>').textbox('setValue',base64_decode(suggestion.data.c_show));
				$('#txtb_gfcs_c_check<?=$time?>').val(base64_decode(suggestion.data.c_id));
			}
		});
	}
</script>