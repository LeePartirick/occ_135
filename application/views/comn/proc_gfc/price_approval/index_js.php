...
function load_table_index<?=$time?>()
{
	...
	var col=[[
        {field:'pa_eq_id_s',title:'设备名称',width:200,halign:'center',align:'left',sortable:true,
            formatter: fun_index_formatter<?=$time?>
        },
        {field:'pa_eq_total',title:'合计',width:200,halign:'center',align:'left',sortable:true,
            formatter: fun_index_formatter<?=$time?>
        },
	]];
	...
	var fcol=[[
				{field:'pa_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'pa_gfc_id_s',title:'财务编号',width:200,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_gfc/price_approval/get_json.html',
		idField:'pa_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
        case 'pa_gfc_id_s':
            ...
            var url='proc_gfc/price_approval/edit/act/2/pa_id/'+base64_decode(row.pa_id);
            value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\''+base64_decode(row.pa_id).substring(0,10)+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_gfc/price_approval/edit/act/'+act;
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_gfc/price_approval/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
