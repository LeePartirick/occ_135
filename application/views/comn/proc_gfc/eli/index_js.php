...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'eli_note',title:'备注',width:200,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
		{field:'eli_id',title:'',checkbox:true,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'eli_name',title:'设备名称',width:200,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_gfc/eli/get_json.html',
		idField:'eli_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'eli_name':
			...
			var url='proc_gfc/eli/edit/act/2/eli_id/'+base64_decode(row.gfc_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'设备清单明细-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_gfc/eli/edit/act/'+act;

	if(op_id)
		url+='/eli_id/'+op_id

	fun_index_win_open<?=$time?>('创建设备清单明细','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn,json_save)
{
	 //批量操作
	$.ajax({
        url:"app/run_back/proc_gfc/eli/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
        ...
        success:function(data){
            ...
            if(json.rs)
            {
            	...
            	fun_win_sys_msg('【设备清单明细批量操作】操作结果:',msg)
