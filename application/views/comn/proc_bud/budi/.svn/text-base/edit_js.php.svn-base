...
<script type="text/javascript">
				...
				else
				{
					...
					else
					{
						...
						if('<?=$act?>'=='<?=STAT_ACT_CREATE?>')
						{
							...
							url+='/act/<?=STAT_ACT_EDIT?>/budi_id/'+json.id
	...
	function load_budi_sub_s<?=$time?>()
	{
		var opt = $('#txtb_budi_sub_s<?=$time?>').textbox('options');

		if(  opt.readonly ) return;
			
		$('#txtb_budi_sub_s<?=$time?>').textbox('textbox').autocomplete({
	        ...
	        onSelect: function (suggestion) {
				$('#txtb_budi_sub_s<?=$time?>').textbox('setValue',base64_decode(suggestion.data.sub_name));
				$('#txtb_budi_sub<?=$time?>').val(base64_decode(suggestion.data.sub_id))

				var sub_name=$('#txtb_budi_name<?=$time?>').textbox('getValue');

				if( ! sub_name )
					$('#txtb_budi_name<?=$time?>').textbox('setValue',base64_decode(suggestion.data.sub_name));
