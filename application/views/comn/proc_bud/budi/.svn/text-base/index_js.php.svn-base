...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'budi_note',title:'备注',width:150,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
		{field:'budi_id',title:'',checkbox:true,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'budi_name',title:'显示科目',width:120,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'budi_ou_show',title:'部门',width:120,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_bud/budi/get_json.html',
		idField:'budi_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'budi_name':
			...
			var url='proc_bud/budi/edit/act/2/budi_id/'+base64_decode(row.budi_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'预算表模型明细-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_bud/budi/edit/act/'+act;
