...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'blp_ou_s',title:'统计部门',width:200,halign:'center',align:'left',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'blp_c_s',title:'审批人',width:200,halign:'center',align:'left',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'blp_note',title:'备注',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]];
	...
	var fcol=[[
		{field:'blp_id',title:'',checkbox:true,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'blp_ppo',title:'审核节点',width:120,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'blp_ppo_num',title:'审核阶段',width:80,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'blp_sub_s',title:'预算科目',width:120,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'blp_sum_start',title:'金额下限',width:100,halign:'center',align:'right',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'blp_sum_end',title:'金额上限',width:100,halign:'center',align:'right',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_bud/bl_ppo/get_json.html',
		idField:'blp_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'blp_ppo':
			...
			var url='proc_bud/bl_ppo/edit/act/2/blp_id/'+base64_decode(row.blp_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'经费流程-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_bud/bl_ppo/edit/act/'+act;

	if(op_id)
		url+='/blp_id/'+op_id

	fun_index_win_open<?=$time?>('创建经费流程','win',url)
}
...
function fun_operate_more_check<?=$time?>(btn)
{
    ...
    {
        case 'edit':
            fun_index_win_open<?=$time?>('批量编辑','win','proc_bud/bl_ppo/edit/act/2/flag_edit_more/1/fun_no_db/fun_edit_more<?=$time?>');
            return;
            break;
    }
...
function fun_operate_more<?=$time?>(list,btn,json_save)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_bud/bl_ppo/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
