...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'t_note',title:'备注',width:150,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
		{field:'t_id',title:'',checkbox:true,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'t_name',title:'流转税',width:150,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_bud/tax/get_json.html',
		idField:'t_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 't_name':
			...
			var url='proc_bud/tax/edit/act/2/t_id/'+base64_decode(row.t_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'流转税-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_bud/tax/edit/act/'+act;

	if(op_id)
		url+='/t_id/'+op_id

	fun_index_win_open<?=$time?>('创建流转税','win',url)
}
...
function fun_operate_more_check<?=$time?>(btn)
{
    ...
    {
        case 'edit':
            fun_index_win_open<?=$time?>('批量编辑','win','proc_bud/tax/edit/act/2/flag_edit_more/1/fun_no_db/fun_edit_more<?=$time?>');
            return;
            break;
    }
...
function fun_operate_more<?=$time?>(list,btn,json_save)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_bud/tax/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
