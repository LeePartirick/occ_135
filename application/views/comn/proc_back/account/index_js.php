...
function load_table_index<?=$time?>()
{
	...
	var col=[[    
				{field:'a_name',title:'显示名称',width:150,halign:'center',align:'center',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'a_status',title:'状态',width:100,halign:'center',align:'center',sortable:true,
					formatter: fun_index_formatter<?=$time?>
			   	},
			   	{field:'a_note',title:'备注',width:200,halign:'center',align:'center',sortable:true,
					formatter: fun_index_formatter<?=$time?>
			   	},
				]];
	...
	var fcol=[[
				{field:'a_id',title:'',checkbox:true,halign:'center',align:'center',
					formatter: fun_index_formatter<?=$time?>
				},
				{field:'a_login_id',title:'账户',width:100,halign:'center',align:'left',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_back/account/get_json.html',
		idField:'a_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'a_login_id':
			...
			var url='proc_back/account/edit/act/2/a_id/'+base64_decode(row.a_id);
...
{
	var url = 'proc_back/account/edit/act/'+act;

	if(op_id)
		url+='/a_id/'+op_id

	fun_index_win_open<?=$time?>('创建账户','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_back/account/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
