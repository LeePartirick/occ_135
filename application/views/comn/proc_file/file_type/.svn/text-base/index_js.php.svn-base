...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		{field:'f_t_parent_s',title:'上级属性',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'f_t_proc_s',title:'适用子系统',width:200,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'f_t_check',title:'相关验证',width:100,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'f_t_note',title:'备注',width:200,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]];
	...
	var fcol=[[
		{field:'f_t_id',title:'',checkbox:true,halign:'center',align:'center',
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'f_t_name',title:'属性名称',width:200,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_file/file_type/get_json.html',
		idField:'f_t_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'f_t_name':
			...
			var url='proc_file/file_type/edit/act/2/f_t_id/'+base64_decode(row.f_t_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'文件属性-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_file/file_type/edit/act/'+act;

	if(op_id)
		url+='/f_t_id/'+op_id

	fun_index_win_open<?=$time?>('创建文件属性','win',url)
}
...
function fun_operate_more_check<?=$time?>(btn)
{
	...
	{
		case 'edit':
			fun_index_win_open<?=$time?>('批量编辑','win','proc_file/file_type/edit/act/2/flag_edit_more/1/fun_no_db/fun_edit_more<?=$time?>');
			return;
			break;
	}
...
//适用子系统查询
function load_f_t_proc_search<?=$time?>()
{
	...
	$(ed.target).combobox('textbox').autocomplete({
		...
		onSelect: function (suggestion) {
			...
			$('#table_search<?=$time?>').propertygrid('updateRow',{
				index: index_s,
				row: {
					value: base64_decode(suggestion.data.p_id),
					value_s: base64_decode(suggestion.data.p_show)
				}
			});
		}
	});
...
//上级属性
function load_f_t_parent_search<?=$time?>()
{
	var row_s =  $('#table_search<?=$time?>').propertygrid('getSelected');
	var index_s = $('#table_search<?=$time?>').propertygrid('getRowIndex',row_s);
	...
	$(ed.target).combobox('textbox').autocomplete({
		...
		onSelect: function (suggestion) {
			...
			$('#table_search<?=$time?>').propertygrid('updateRow',{
				index: index_s,
				row: {
					value: base64_decode(suggestion.data.f_t_id),
					value_s: base64_decode(suggestion.data.f_t_name)
				}
			});
		}
	});
