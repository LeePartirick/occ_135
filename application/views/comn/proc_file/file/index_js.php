...
function load_table_index<?=$time?>()
{
	...
	var col=[[
		...
		{field:'f_type_s',title:'文件属性',width:200,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
		{field:'f_note',title:'备注',width:150,halign:'center',align:'center',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]];
	...
	var fcol=[[
		...
		{field:'f_name',title:'文件名',width:200,halign:'center',align:'left',sortable:true,
			formatter: fun_index_formatter<?=$time?>
		},
	]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_file/file/get_json.html',
		idField:'f_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_operate_more_check<?=$time?>(btn)
{
	...
	{
		case 'edit':
			fun_index_win_open<?=$time?>('批量编辑','win','proc_file/file/edit/act/2/flag_edit_more/1/fun_no_db/fun_edit_more<?=$time?>');
			return;
			break;
	}
