...
function load_table_index<?=$time?>()
{
	...
	var col=[[    
		{field:'c_sex',title:'性别',width:80,halign:'center',align:'center',sortable:true,
	...
	var fcol=[[
				...
				{field:'c_login_id',title:'关联账户',width:120,halign:'center',align:'center',sortable:true,
					formatter: fun_index_formatter<?=$time?>
				},
			]]
	...
	$('#table_index<?=$time?>').datagrid({
		...
		url:'proc_contact/contact/get_json<?=$url_param?>.html',
		idField:'c_id',
		sort:'<?=$sort?>',
		order:'<?=$order?>',
		rownumbers:true,
...
function fun_index_formatter<?=$time?>(value,row,index){
	...
	switch(this.field)
	{
		case 'c_name':
			...
			var url='proc_contact/contact/edit/act/2/c_id/'+base64_decode(row.c_id);
			value='<a href="javascript:void(0);" class="link" onClick="fun_index_win_open<?=$time?>(\'联系人-'+value+'\',\'win\',\''+url+'\');">'+value+'</a>';
...
{
	var url = 'proc_contact/contact/edit/act/'+act;

	if(op_id)
		url+='/c_id/'+op_id

	fun_index_win_open<?=$time?>('创建联系人','win',url)
}
...
function fun_operate_more<?=$time?>(list,btn)
{
	 //保存配置
	$.ajax({
        url:"app/run_back/proc_contact/contact/fun_operate_more.html",
        type:"POST",
        data:{
			list : list,
			btn : btn,
        ...
        success:function(data){
            ...
            if(json.rs)
            {
            	...
            	fun_win_sys_msg('【联系人批量操作】操作结果:',msg)
