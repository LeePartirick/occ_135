...
class Doc extends CI_Controller {
	...
	public function get_json_jy(){
		...
		if(element('search_jy_info',$data_get))
		{
			$arr_search['from']='oa_doc_borrow docb 
								LEFT JOIN sys_link l ON
								(docb.docb_id=l.op_id)';
			$arr_search['where']=" AND link_id IN 
								(SELECT doci_id FROM oa_doc doc 
								LEFT JOIN oa_doc_item doci 
								ON(doc.doc_id=doci.doc_id) WHERE doc.doc_id=?) 
								GROUP BY docb_id";
			$arr_search['value'][]=$data_get['search_jy_info'];
		}else{
			exit;
		}
