...
class Role extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('proc_back/m_role');
	...
	public function index()
	{
		...
		$data_out['url']='proc_back-role-index';
		
		$data_out['field_search_start']='role_name,role_default,role_note';
		$data_out['field_search_rule_default']='{"field":"role_name","field_s":"角色名称","table":"sys_role","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"role_name","text":"角色名称","rule":{"field":"role_name","field_s":"角色名称","table":"sys_role","value":"'.element('role_name',$data_get).'","rule":"like","group":"search","editor":"text"}},
        {"id":"role_default","text":"默认角色","rule":{"field":"role_default","field_s":"默认角色","table":"sys_role","value":"'.element('role_default',$data_get).'","rule":"in","group":"search","editor":{
	        	"type":"combotree",
					"options":{
					"valueField": "id",
					"textField": "text",
					"panelHeight":"auto",
					"multiple":"true",
					"data":['.get_json_for_arr($GLOBALS['m_role']['text']['role_default']).'] 
					}
	        }}},
        {"id":"role_note","text":"角色描述","rule":{"field":"role_note","field_s":"角色描述","table":"sys_role","value":"'.element('role_note',$data_get).'","rule":"like","group":"search","editor":"text"}}';
		
		$data_out['field_search_value_dispaly']= array(
			'role_default'=>$GLOBALS['m_role']['text']['role_default'],
		);
		...
		/************载入视图 ****************/
		$arr_view[]='proc_back/role/index';
		$arr_view[]='proc_back/role/index_js';
	...
	public function get_json()
	{
		...
		$arr_search['field']='role_id,role_name,role_default,role_note';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					if(isset($GLOBALS['m_role']['text'][$f]) && isset($GLOBALS['m_role']['text'][$f][$v[$f]]) )
					$v[$f]=$GLOBALS['m_role']['text'][$f][$v[$f]];
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				foreach ($col as $v) {
					...
					switch ($v) {
						case 'role_default':
							$value = array_search($value,$GLOBALS['m_role']['text']['role_default']);
						break;
					}
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_role->load($arr_get,$arr_post);
