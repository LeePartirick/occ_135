 ...
 *
 */
class Price_approval extends CI_Controller {

	
	public function __construct()
    {
        parent::__construct();
        $this->load->model('proc_gfc/m_price_approval');
	...
	public function index()
	{
		...
		$data_out['title']='自研产品索引';
		$data_out['time']=time();
		if( ! empty(element('time', $data_get)) )
		$data_out['time']=element('time', $data_get);
		
		$data_out['url']='proc_gfc-price_approval-index';
		
		$data_out['field_search_start']='pa_gfc_id';
		$data_out['field_search_rule_default']='{"field":"pa_gfc_id","field_r":"pa_gfc_id","field_s":"财务编号","table":"pm_bud_price_approval","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"pa_gfc_id","text":"财务编号","rule":{"field":"pa_eq_id","field_r":"pa_gfc_id","field_s":"财务编号","table":"pm_bud_price_approval","value":"'.element('pa_eq_id',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_gfc/price_approval/index';
		$arr_view[]='proc_gfc/price_approval/index_js';
	...
	public function get_json()
	{
		...
		$arr_field_search[]='pa_id';

		$arr_search['field']=implode(',', $arr_field_search);

		$arr_search['from']='pm_given_financial_code gfc
		                    LEFT JOIN pm_bud_price_approval pa
		                    ON(gfc.gfc_id=pa.pa_gfc_id)';
        $arr_search['where']=" AND pa_eq_id!='' AND pa_eq_id IS NOT NULL GROUP BY gfc.gfc_id";
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					switch($f){
                        case 'pa_gfc_id':
                            $row_f.='"pa_gfc_id_s":"'.fun_urlencode($this->m_base->get_field_where('pm_given_financial_code','gfc_finance_code'," AND gfc_id ='{$v[$f]}'")).'",';
                            break;
                        case 'GROUP_CONCAT(pa_eq_id)';
                            $eq_id=explode(',',$v[$f]);
                            if(count($eq_id)>0){
                                $eq_name='';
                                foreach($eq_id as $k1=>$v1){
                                    $eq_name.=$this->m_base->get_field_where('sm_equipment','eq_name'," AND eq_id='{$eq_id[$k1]}'").',';
                                }
                            }
                            $eq_name=trim($eq_name,',');
                            $row_f.='"pa_eq_id_s":"'.fun_urlencode($eq_name).'",';
                            break;
                        case 'SUM(pa_eq_total)':
                            $row_f.='"pa_eq_total":"'.fun_urlencode($v[$f]).'",';
//                            var_dump($v[$f]);exit;
                            break;
						default:
							if(isset($GLOBALS['m_price_approval']['text'][$f]) && isset($GLOBALS['m_price_approval']['text'][$f][$v[$f]]) )
								$v[$f]=$GLOBALS['m_price_approval']['text'][$f][$v[$f]];
					}
	...
	public function fun_operate_more()
	{
		...
		if(count($list) > 0)
		{
			...
			foreach ($list as $k => $v) {
				...
				$arr_rtn=$this->m_price_approval->load($arr_get,$arr_post);
