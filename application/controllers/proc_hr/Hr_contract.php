 ...
 *
 */
class Hr_contract extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='合同索引';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_contract-index';
		
		$data_out['field_search_start']='c_id,cont_hr_type,cont_hr_type_work,cont_type,cont_year_1,cont_year_2';
		$data_out['field_search_rule_default']='{"field":"c_id","field_s":"员工姓名","table":"hr_contract","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"c_id","text":"员工姓名","rule":{"field":"c_id","field_s":"员工姓名","table":"hr_contract","value":"'.element('c_id',$data_get).'","rule":"like","group":"search","editor":"text"}},
		{"id":"cont_year_1","text":"合同年限-大于","rule":{"field":"cont_year_1","field_r":"cont_year","field_s":"合同年限-大于","table":"hr_contract","value":"'.element('cont_year',$data_get).'","rule":">=","group":"search","editor":{
			"type":"numberbox"
		}}},
		{"id":"cont_year_2","text":"合同年限-小于","rule":{"field":"cont_year_2","field_r":"cont_year","field_s":"合同年限-小于","table":"hr_contract","value":"'.element('cont_year',$data_get).'","rule":"<=","group":"search","editor":{
			"type":"numberbox"
		}}},
		{"id":"cont_hr_type","text":"人员类别","rule":{"field":"cont_hr_type","field_s":"人员类别","table":"hr_contract","value":"'.element('cont_hr_type',$data_get).'","rule":"in","group":"search","editor":{
			"type":"combobox",
			"options":{
				"valueField": "id",
				"textField": "text",
		...
		{"id":"cont_hr_type_work","text":"用工形式","rule":{"field":"cont_hr_type_work","field_s":"用工形式","table":"hr_contract","value":"'.element('cont_hr_type_work',$data_get).'","rule":"in","group":"search","editor":{
			"type":"combobox",
			"options":{
				"valueField": "id",
				"textField": "text",
		...
		{"id":"cont_type","text":"合同性质","rule":{"field":"cont_type","field_s":"合同性质","table":"hr_contract","value":"'.element('cont_type',$data_get).'","rule":"in","group":"search","editor":{
			"type":"combobox",
			"options":{
				"valueField": "id",
				"textField": "text",
				"panelHeight":"auto",
				"multiple":"true",
				"data":['.get_json_for_arr($GLOBALS['m_hr']['text']['cont_type']).']
				}
		}}}
		';
		
		$data_out['field_search_value_dispaly']= array(
			'cont_hr_type'=>$GLOBALS['m_hr']['text']['hr_type'],
			'cont_hr_type_work'=>$GLOBALS['m_hr']['text']['hr_type_work'],
			'cont_type'=>$GLOBALS['m_hr']['text']['cont_type'],
		);
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_contract/index';
		$arr_view[]='proc_hr/hr_contract/index_js';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
                        case 'c_id':
                            $row_f.='"c_id_s":"'.fun_urlencode($this->m_base->get_field_where('sys_contact','c_name'," AND c_id ='{$v[$f]}'")).'",';
                            break;
                        case 'cont_hr_type':
                            $v[$f] = $GLOBALS['m_hr']['text']['hr_type'][$v[$f]];
                            break;
                        case 'cont_hr_type_work':
                            $v[$f] = $GLOBALS['m_hr']['text']['hr_type_work'][$v[$f]];
                            break;
						default:
							if(isset($GLOBALS['m_hr_contract']['text'][$f]) && isset($GLOBALS['m_hr_contract']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_contract']['text'][$f][$v[$f]];
					}
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				foreach ($col as $v) {
					...
					switch($v){
						case 'cont_hr_type':
							$value = array_search($value,$GLOBALS['m_hr_contract']['text']['cont_hr_type']);
							break;
						case 'cont_hr_type_work':
							$value = array_search($value,$GLOBALS['m_hr_contract']['text']['cont_hr_type_work']);
							break;
						case 'cont_type':
							$value = array_search($value,$GLOBALS['m_hr_contract']['text']['cont_type']);
							break;
					}
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_contract->load($arr_get,$arr_post);
