 ...
 *
 */
class Hr_tec extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='技术方向索引';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_tec-index';
		
		$data_out['field_search_start']='tec_name,tec_note';
		$data_out['field_search_rule_default']='{"field":"tec_name","field_s":"技术方向","table":"hr_tec","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"tec_name","text":"技术方向","rule":{"field":"tec_name","field_s":"技术方向","table":"hr_tec","value":"'.element('tec_name',$data_get).'","rule":"like","group":"search","editor":"text"}}
		,{"id":"tec_note","text":"描述","rule":{"field":"tec_note","field_s":"描述","table":"hr_tec","value":"'.element('tec_note',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_tec/index';
		$arr_view[]='proc_hr/hr_tec/index_js';
	...
	public function get_json()
	{
		...
		$arr_field_search[]='tec_id';
		
		$arr_search['field']=implode(',', $arr_field_search);
		
		$arr_search['from']='hr_tec';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_tec']['text'][$f]) && isset($GLOBALS['m_hr_tec']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_tec']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_tec->load($arr_get,$arr_post);
