 ...
 *
 */
class Hr_zw extends CI_Controller {
	...
	public function __construct()
    {
        ...
        $this->load->model('proc_hr/m_hr_zw');
    }
	...
	public function index()
	{
		...
		$data_out['title']='职务索引';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_zw-index';
		
		$data_out['field_search_start']='zw_name,zw_note';
		$data_out['field_search_rule_default']='{"field":"zw_name","field_s":"职务名称","table":"hr_zw","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"zw_name","text":"职务名称","rule":{"field":"zw_name","field_s":"职务名称","table":"hr_zw","value":"'.element('zw_name',$data_get).'","rule":"like","group":"search","editor":"text"}}
		,{"id":"zw_note","text":"职务描述","rule":{"field":"zw_note","field_s":"职务描述","table":"hr_zw","value":"'.element('zw_note',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_zw/index';
		$arr_view[]='proc_hr/hr_zw/index_js';
	...
	public function get_json()
	{
		...
		$arr_field_search[]='zw_id';
		
		$arr_search['field']=implode(',', $arr_field_search);
		
		$arr_search['from']='hr_zw';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_zw']['text'][$f]) && isset($GLOBALS['m_hr_zw']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_zw']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_zw->load($arr_get,$arr_post);
