 ...
 *
 */
class Hr_work extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='工作经历';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_work-index';
		
		$data_out['field_search_start']='hr_w_org';
		$data_out['field_search_rule_default']='{"field":"hr_w_org","field_s":"公司名称","table":"hr_work","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"hr_w_org","text":"公司名称","rule":{"field":"hr_w_org","field_s":"公司名称","table":"hr_work","value":"'.element('hr_w_org',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_work/index';
		$arr_view[]='proc_hr/hr_work/index_js';
	...
	public function get_json()
	{
		...
		{
			$arr_search['sort']='hr_w_time_start';
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="hr_w_time_start";
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_work']['text'][$f]) && isset($GLOBALS['m_hr_work']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_work']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_work->load($arr_get,$arr_post);
