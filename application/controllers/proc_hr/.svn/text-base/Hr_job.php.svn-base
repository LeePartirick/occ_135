 ...
 *
 */
class Hr_job extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='职位索引';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_job-index';
		
		$data_out['field_search_start']='job_code,job_name,job_note';
		$data_out['field_search_rule_default']='{"field":"job_name","field_s":"职位名称","table":"","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"job_code","text":"编号","rule":{"field":"job_code","field_s":"编号","table":"","value":"'.element('job_code',$data_get).'","rule":"like","group":"search","editor":"text"}}
		,{"id":"job_name","text":"职位名称","rule":{"field":"job_name","field_s":"职位名称","table":"","value":"'.element('job_name',$data_get).'","rule":"like","group":"search","editor":"text"}}
		,{"id":"job_note","text":"备注","rule":{"field":"job_note","field_s":"备注","table":"","value":"'.element('job_note',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_job/index';
		$arr_view[]='proc_hr/hr_job/index_js';
	...
	public function get_json()
	{
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="job_code,job_name";
		...
		$arr_field_search[]='job_id';
		
		$arr_search['field']=implode(',', $arr_field_search);
		
		$arr_search['from']='hr_job';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					if(isset($GLOBALS['m_hr_job']['text'][$f]) && isset($GLOBALS['m_hr_job']['text'][$f][$v[$f]]) )
					$v[$f]=$GLOBALS['m_hr_job']['text'][$f][$v[$f]];
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_job->load($arr_get,$arr_post);
