 ...
 *
 */
class Hr_reward extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='奖惩记录';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_reward-index';
		
		$data_out['field_search_start']='hr_rp_doc_code';
		$data_out['field_search_rule_default']='{"field":"hr_rp_doc_code","field_s":"文件号","table":"hr_reward","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"hr_rp_doc_code","text":"文件号","rule":{"field":"hr_rp_doc_code","field_s":"文件号","table":"hr_reward","value":"'.element('hr_rp_doc_code',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_reward/index';
		$arr_view[]='proc_hr/hr_reward/index_js';
	...
	public function get_json()
	{
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="hr_rp_doc_code";
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					switch ($f)
					{
						case 'hr_rp_ou_approve':

						$v['hr_rp_ou_approve_s']=$this->m_base->get_field_where('sys_org','o_name',"AND o_id = '".element($f,$v)."'");
						$row_f.='"'.$f.'_s":"'.fun_urlencode($v['hr_rp_ou_approve_s']).'",';

						break;
						default:
							if(isset($GLOBALS['m_hr_reward']['text'][$f]) && isset($GLOBALS['m_hr_reward']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_reward']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_reward->load($arr_get,$arr_post);
