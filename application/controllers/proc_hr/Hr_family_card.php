 ...
 *
 */
class Hr_family_card extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_family_card-index';
		
		$data_out['field_search_start']='hr_fcard_name';
		$data_out['field_search_rule_default']='{"field":"hr_fcard_name","field_s":"证件名称","table":"hr_family_card","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"hr_fcard_name","text":"证件名称","rule":{"field":"hr_fcard_name","field_s":"证件名称","table":"hr_family_card","value":"'.element('hr_fcard_name',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_family_card/index';
		$arr_view[]='proc_hr/hr_family_card/index_js';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_family_card']['text'][$f]) && isset($GLOBALS['m_hr_family_card']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_family_card']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_family_card->load($arr_get,$arr_post);
