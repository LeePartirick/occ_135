 ...
 *
 */
class Hr_card extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='卡帐信息';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_card-index';
		
		$data_out['field_search_start']='hr_card_code';
		$data_out['field_search_rule_default']='{"field":"hr_card_code","field_s":"卡号","table":"hr_card","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"hr_card_code","text":"卡号","rule":{"field":"hr_card_code","field_s":"卡号","table":"hr_card","value":"'.element('hr_card_code',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_card/index';
		$arr_view[]='proc_hr/hr_card/index_js';
	...
	public function get_json()
	{
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="hr_card_code";
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_card']['text'][$f]) && isset($GLOBALS['m_hr_card']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_card']['text'][$f][$v[$f]];
					}
            ...
            while (TRUE)
            {
                ...
                foreach ($col as $v) {
                    ...
                    switch ($v) {
                        ...
                        case 'hr_card_code':
                        	$value = sctonum($value);
                            $hr_card_id =  $this->m_base->get_field_where('hr_card','hr_card_id'," AND hr_card_type = '{$arr_post['content']['hr_card_type']}' AND c_id = '{$arr_get['c_id']}'");
							
                            if( $hr_card_id )
                            {
                            	$arr_get['act'] = STAT_ACT_EDIT;
                            	$arr_get['hr_card_id'] = $hr_card_id;
                            }
                ...
                $arr_post['btn']='save';
                $arr_rtn=$this->m_hr_card->load($arr_get,$arr_post);
