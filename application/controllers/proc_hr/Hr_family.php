 ...
 *
 */
class Hr_family extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='家庭信息';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_family-index';
		
		$data_out['field_search_start']='hr_f_person';
		$data_out['field_search_rule_default']='{"field":"hr_f_person","field_s":"姓名","table":"hr_family","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"hr_f_person","text":"姓名","rule":{"field":"hr_f_person","field_s":"姓名","table":"hr_family","value":"'.element('hr_f_person',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_family/index';
		$arr_view[]='proc_hr/hr_family/index_js';
	...
	public function get_json()
	{
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="hr_f_relation";
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_family']['text'][$f]) && isset($GLOBALS['m_hr_family']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_family']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_family->load($arr_get,$arr_post);
