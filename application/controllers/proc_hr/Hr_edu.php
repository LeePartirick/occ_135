 ...
 *
 */
class Hr_edu extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='卡帐信息';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_edu-index';
		
		$data_out['field_search_start']='hr_edu_school';
		$data_out['field_search_rule_default']='{"field":"hr_edu_school","field_s":"学校","table":"hr_edu","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"hr_edu_school","text":"学校","rule":{"field":"hr_edu_school","field_s":"学校","table":"hr_edu","value":"'.element('hr_edu_school',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_edu/index';
		$arr_view[]='proc_hr/hr_edu/index_js';
	...
	public function get_json()
	{
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="hr_edu_school";
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_edu']['text'][$f]) && isset($GLOBALS['m_hr_edu']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_edu']['text'][$f][$v[$f]];
					}
