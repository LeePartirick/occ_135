 ...
 *
 */
class Hr_family_crime extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='家庭信息';
		$data_out['time']=time();
		$data_out['url']='proc_hr-hr_family_crime-index';
		
		$data_out['field_search_start']='hr_fc_description';
		$data_out['field_search_rule_default']='{"field":"hr_fc_description","field_s":"描述","table":"hr_family_crime","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"hr_fc_description","text":"描述","rule":{"field":"hr_fc_description","field_s":"描述","table":"hr_family_crime","value":"'.element('hr_fc_description',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_hr/hr_family_crime/index';
		$arr_view[]='proc_hr/hr_family_crime/index_js';
	...
	public function get_json()
	{
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="hr_fc_description";
		...
		$arr_field_search[]='hr_fc_id';
		
		$arr_search['field']=implode(',', $arr_field_search);
		
		$arr_search['from']='hr_f';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						default:
							if(isset($GLOBALS['m_hr_family_crime']['text'][$f]) && isset($GLOBALS['m_hr_family_crime']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_hr_familycrime']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_hr_family_crime->load($arr_get,$arr_post);
