...
class Invoice_type extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('proc_bud/m_invoice_type');
	...
	public function index()
	{
		...
		$data_out['title']='发票类型索引';
		$data_out['time']=time();
		$data_out['url']='proc_bud-invoice_type-index';
		
		$data_out['field_search_start']='it_name';
		$data_out['field_search_rule_default']='{"field":"it_name","field_s":"发票类型","table":"","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"it_name","text":"发票类型","rule":{"field":"it_name","field_s":"发票类型","table":"","value":"'.element('it_name',$data_get).'","rule":"like","group":"search","editor":"text"}},
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_bud/invoice_type/index';
		$arr_view[]='proc_bud/invoice_type/index_js';
	...
	public function get_json()
	{
		...
		if(element('search_t_id', $data_get))
		{
			$t_invoice_type = $this->m_base->get_field_where('fm_tax_type','t_invoice_type',"AND t_id = '{$data_get['search_t_id']}'");
			$arr_search['where']=' AND it_id IN ? ';
			if(element('search_it_id', $data_get))
			$t_invoice_type.=','.$data_get['search_it_id'];
			
			$arr_search['value'][]=explode(',', $t_invoice_type);
		}
		
		if( $query )
		{
			if( ! $field_q ) $field_q="it_name";
		...
		$arr_field_search[]='it_id';

		$arr_search['field']=implode(',', $arr_field_search);
		
		$arr_search['from']='fm_invoice_type';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					switch($f){
						default:
							if(isset($GLOBALS['m_invoice_type']['text'][$f]) && isset($GLOBALS['m_invoice_type']['text'][$f][$v[$f]]) )
								$v[$f]=$GLOBALS['m_invoice_type']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_invoice_type->load($arr_get,$arr_post);
