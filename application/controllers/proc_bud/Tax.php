...
class Tax extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('proc_bud/m_tax');
	...
	public function index()
	{
		...
		$data_out['title']='流转税索引';
		$data_out['time']=time();
		$data_out['url']='proc_bud-tax-index';
		
		$data_out['field_search_start']='t_name';
		$data_out['field_search_rule_default']='{"field":"t_name","field_s":"流转税","table":"","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"t_name","text":"流转税","rule":{"field":"t_name","field_s":"流转税","table":"","value":"'.element('t_name',$data_get).'","rule":"like","group":"search","editor":"text"}},
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_bud/tax/index';
		$arr_view[]='proc_bud/tax/index_js';
	...
	public function get_json()
	{
		...
		$arr_field_search[]='t_id';

		$arr_search['field']=implode(',', $arr_field_search);
		
		$arr_search['from']='fm_tax_type';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					switch($f){
						default:
							if(isset($GLOBALS['m_tax']['text'][$f]) && isset($GLOBALS['m_tax']['text'][$f][$v[$f]]) )
								$v[$f]=$GLOBALS['m_tax']['text'][$f][$v[$f]];
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_tax->load($arr_get,$arr_post);
