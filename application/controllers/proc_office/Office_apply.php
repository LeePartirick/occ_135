 ...
 *
 */
class Office_apply extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='信息系统申请索引';
		$data_out['time']=time();
		$data_out['url']='proc_office-office_apply-index';

		$data_out['field_search_start']='offa_code,offa_c_apply,offa_c_id,ppo';
		$data_out['field_search_rule_default']='{"field":"offa_code","field_s":"单据编号","table":"","value":"","rule":"like","group":"search","editor":"text"}';
		...
		,{"id":"offa_c_apply","text":"申请人","rule":{"field":"offa_c_apply","field_s":"申请人","table":"","value":"'.element('offa_c_apply',$data_get).'","rule":"=","group":"search",
		 "editor":{
			"type":"combobox",
			"options":{
				"hasDownArrow":"",
				"panelHeight":"0",
				"start_fun":"load_offa_c_apply'.$data_out['time'].'()"
			}
		}}}
		,{"id":"offa_c_id","text":"系统所有人","rule":{"field":"offa_c_id","field_s":"系统所有人","table":"","value":"'.element('offa_c_id',$data_get).'","rule":"=","group":"search",
		 "editor":{
			"type":"combobox",
			"options":{
				"hasDownArrow":"",
				"panelHeight":"0",
				"start_fun":"load_offa_c_id'.$data_out['time'].'()"
			}
		}}}
		,{"id":"ppo","text":"流程节点","rule":{"field":"ppo","field_s":"流程节点","table":"","value":"'.element('ppo',$data_get).'","rule":"in","group":"search","editor":{
        	"type":"combotree",
				...
				"panelHeight":"auto",
				"multiple":"true",
				"data":['.get_json_for_arr($GLOBALS['m_office_apply']['text']['ppo']).']
				}
        }}}
        ';

		$data_out['field_search_value_dispaly']= array(
            'ppo'=>$GLOBALS['m_office_apply']['text']['ppo'],
		...
		/************载入视图 ****************/
		$arr_view[]='proc_office/office_apply/index';
		$arr_view[]='proc_office/office_apply/index_js';
	...
	public function get_json()
	{
		...
		{
			case 'offa_c_apply_s':
				$arr_search['sort']='offa_c_apply';
				break;
			case 'offa_c_id_s':
				$arr_search['sort']='offa_c_id';
				break;
		}
		...
		$arr_field_search[]='offa_id';

		$arr_search['field']=implode(',', $arr_field_search);

		$arr_search['from']='oa_office_apply';
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					if(isset($GLOBALS['m_office_apply']['text'][$f]) && isset($GLOBALS['m_office_apply']['text'][$f][$v[$f]]) )
						$v[$f]=$GLOBALS['m_office_apply']['text'][$f][$v[$f]];
...
//					}
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_office_apply->load($arr_get,$arr_post);
