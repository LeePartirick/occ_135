 ...
 *
 */
class Oa_offai extends CI_Controller {
	...
	public function index()
	{
		...
		$data_out['title']='工作经历';
		$data_out['time']=time();
		$data_out['url']='proc_office-oa_offai-index';
		
		$data_out['field_search_start']='offai_offi_id';
		$data_out['field_search_rule_default']='{"field":"offai_offi_id","field_s":"信息系统","table":"oa_office_item","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"offai_offi_id","text":"信息系统","rule":{"field":"offai_offi_id","field_s":"信息系统","table":"oa_office_item","value":"'.element('offai_offi_id',$data_get).'","rule":"like","group":"search","editor":"text"}}
		';
		...
		/************载入视图 ****************/
		$arr_view[]='proc_office/oa_offai/index';
		$arr_view[]='proc_office/oa_offai/index_js';
	...
	public function get_json()
	{
		...
		{
			$arr_search['sort']='offai_person_start';
		...
		if( element('flag_start', $data_get) )
		{
		...
		if( $query )
		{
			if( ! $field_q ) $field_q="offai_person_start";
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					{
						case 'offai_offi_id':
							$v[$f.'_s'] = $this->m_base->get_field_where('oa_office','offi_name'," AND offi_id = '{$v[$f]}'");
						...
						case 'offai_person_end':
							...
							break;
						default:
							if(isset($GLOBALS['m_oa_offai']['text'][$f]) && isset($GLOBALS['m_oa_offai']['text'][$f][$v[$f]]) )
							$v[$f]=$GLOBALS['m_oa_offai']['text'][$f][$v[$f]];
					}
