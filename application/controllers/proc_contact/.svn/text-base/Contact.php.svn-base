...
class Contact extends CI_Controller {
	...
	public function index()
	{
		...
		/************模板赋值***********/
		$data_out['url_param'] = $this->m_base->get_url_param_of_data_get($data_get);
		...
		$data_out['title']='联系人索引';
		$data_out['time']=time();
		$data_out['url']='proc_contact-contact-index';
		
		$data_out['field_search_start']='c_name,c_login_id,c_tel,c_org';
		$data_out['field_search_rule_default']='{"field":"c_name","field_s":"姓名","table":"sys_contact","value":"","rule":"like","group":"search","editor":"text"}';
		$data_out['field_search_rule']='
		{"id":"c_name","text":"姓名","rule":{"field":"c_name","field_s":"姓名","table":"sys_contact","value":"'.element('c_name',$data_get).'","rule":"like","group":"search","editor":"text"}},
		{"id":"c_login_id","text":"关联账户","rule":{"field":"c_login_id","field_s":"关联账户","table":"sys_contact","value":"'.element('c_login_id',$data_get).'","rule":"like","group":"search","editor":"text"}},
		{"id":"c_tel","text":"手机","rule":{"field":"c_tel","field_s":"手机","table":"sys_contact","value":"'.element('c_tel',$data_get).'","rule":"like","group":"search","editor":"text"}},
		{"id":"c_email","text":"Email","rule":{"field":"c_email","field_s":"Email","table":"sys_contact","value":"'.element('c_email',$data_get).'","rule":"like","group":"search","editor":"text"}},
		{"id":"c_sex","text":"性别","rule":{"field":"c_sex","field_s":"性别","table":"sys_contact","value":"'.element('c_sex',$data_get).'","rule":"in","group":"search","editor":{
	        	"type":"combotree",
					"options":{
					"valueField": "id",
					"textField": "text",
	    ...
	    ,{"id":"c_org","text":"所属机构","rule":{"field":"c_org","field_s":"所属机构","table":"sys_contact","value":"'.element('c_org',$data_get).'","rule":"in","group":"search","editor":{
	        	"type":"combobox",
					"options":{
					"hasDownArrow":"",
					"panelHeight":"0",
		...
		/************载入视图 ****************/
		$arr_view[]='proc_contact/contact/index';
		$arr_view[]='proc_contact/contact/index_js';
	...
	public function get_json()
	{
		...
		if(element('search_c_type',$data_get))
		{
		...
		if (count($rs['content']) > 0)
		{
			foreach($rs['content'] as $k => $v)
			{
				...
				foreach ($arr_field_search as $f) {
					...
					if(isset($GLOBALS['m_contact']['text'][$f]) && isset($GLOBALS['m_contact']['text'][$f][$v[$f]]) )
					$v[$f]=$GLOBALS['m_contact']['text'][$f][$v[$f]];
					
					switch ($f)
					{
						case 'c_org':
							$row_f.='"c_org_s":"'.fun_urlencode($this->m_base->get_field_where('sys_org','o_name'," AND o_id ='{$v[$f]}'")).'",';
							break;
					}
	...
	public function import()
	{
		...
		if($row_num)
		{
			...
			while (TRUE)
			{
				...
				$arr_post['btn']='save';
				$arr_rtn=$this->m_contact->load($arr_get,$arr_post);
